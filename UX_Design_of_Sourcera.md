...
| `feedback.duration.score_override_highlight` | 5s | Static yellow visibility dwell for a replacement Score grade; this is not an animation and remains visible for the same duration when reduced motion is active. |
...
- "Other" tile is the sixth in the grid; selecting it expands an inline single-line free-text input bounded by Master Spec §39 `Workspace.intake_freetext_label` with a "Continue" button. The free-text input renders inline below the tile grid; the tiles remain visible.
...
- Validation: length per Master Spec §39 `Workspace.intake_freetext_label`; HTML / script injection sanitized at write per §32 input validation.
...
│            │   │ Deadline:    {deadline_date} (in {default_deadline_days} days) ·│                  │
│            │   │              Change                         │                  │
│            │   │                                             │                  │
│            │   │              [ Got it ]                     │                  │
│            │   └─────────────────────────────────────────────┘                  │
...
- The "What we filled in for you" callout (§13.12.7) is pinned to the top-right of the content region for the `callout.dismiss.timeout` duration or until dismissed.
- After dismissal (or after the timeout elapses), the callout collapses into a quiet "What we filled in for you" link in the §11.4 persistent UI re-openable for the life of the Workspace.
...
6. **Dismissal.** Clicking the "Got it" button or the X icon (top-right of the callout) dismisses the callout immediately; reaching `callout.dismiss.timeout` dismisses it identically. Both paths trigger the slide-out motion using `motion.duration.medium` + `motion.ease.standard`, and the callout collapses into a quiet "What we filled in for you" link in the §11.4 persistent UI.
...
| `idle-with-timer` | Mounted, timeout not elapsed | Callout visible, auto-dismiss timer running | |
...
| `dismissed-auto` | `callout.dismiss.timeout` elapsed | Slide-out (`motion.duration.medium`); collapses to §11.4 link | Same animation as explicit |
...
| IF-03 | Selecting a default tile invokes the §13.12.4 materialization API, navigates to the materialized Workspace, and renders the `IntakeFillCallout` for `callout.dismiss.timeout` or until dismissed. Materialization completes within p95 ≤ 2.5s for Free-tier post-truncation and p95 ≤ 4.5s for Starter+ per Master Spec §44 / §13.12.10.2 AC #9. | E2E test + p95 latency assertion |
| IF-04 | Selecting "Other" expands an inline FormField with length per Master Spec §39 `Workspace.intake_freetext_label`; Continue invokes materialization with `vertical_slug=other` and `intake_freetext_label=<input>`; the resulting Workspace's audit trail records both fields per §13.12.4 step 8. | E2E test + audit-log assertion |
| IF-05 | Selecting the "blank Workspace" link materializes a Workspace with no Use Cases, Requirements, or longlist and the Master Spec §4.5.9 `default_deadline_days` column default; no callout renders. | E2E test |
...
| IF-09 | Auto-dismiss fires at `callout.dismiss.timeout`; explicit dismiss fires immediately on Got it / X / Esc. Both paths slide out using `motion.duration.medium` (instant snap when `prefers-reduced-motion: reduce` is set) and collapse to a §11.4 "What we filled in for you" link. | Visual regression + reduced-motion test |
...
**Suppressed-surface QA hooks.** Master Spec §44.6.1.A is authoritative. Every component above uses its matching `[data-solo-suppressed-surface="..."]` hook; the Solo route must omit the component entirely from DOM, portals, mobile sheets, and the accessibility tree. CSS-only hiding is not conforming. UX tests import the exact ten-selector manifest from the Master-Spec-backed fixture and do not maintain a second selector list.
