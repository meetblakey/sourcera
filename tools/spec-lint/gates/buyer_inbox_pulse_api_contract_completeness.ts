 ...
 *
 * Assertion: every buyer §20 Inbox / Pulse operation resolves to a complete
 * §32.10.3.B API contract in the spec tree.
 */
...
const GATE_ID = "buyer_inbox_pulse_api_contract_completeness";

const SECTION_20_TOKENS = [
  ...
  "`GET /v1/workspaces/{workspace_id}/inbox/{item_id}`",
  "`POST /v1/workspaces/{workspace_id}/inbox/{item_id}/read`",
  "`POST /v1/workspaces/{workspace_id}/inbox/{item_id}/dismiss`",
  "`POST /v1/workspaces/{workspace_id}/inbox/{item_id}/restore`",
  "`POST /v1/workspaces/{workspace_id}/inbox/mark-all-read`",
  "`GET /v1/workspaces/{workspace_id}/inbox?type=&read_state=&delivery_state=`",
  "Desktop and tablet may initiate the async CSV export through §32.10.3.B",
  "the API remains §32.10.3.B",
...
const SECTION_32_TOKENS = [
  ...
  "**Response schema bindings.**",
  "List endpoints return the §32.3 cursor envelope with top-level `data` and `pagination {next_cursor, has_more, limit}`",
  ...
  "**Acceptance criteria.**",
  "Every endpoint MUST reject seller-console tokens, cross-Org tokens, and cross-Workspace tokens before resolving entity existence.",
  "Every state-mutating endpoint MUST require `Idempotency-Key`; same-key / same-body replay returns the original response",
  "Preference patch MUST delegate quiet hours, DND, and entity mute semantics to §29.3 and reject SMS channel keys until the SMS contract is authored.",
];

const M5_ROW_TOKENS = [
  "Every §20 Inbox / Pulse operation named in the surface resolves to a §32.10.3.B endpoint",
  "auth scope, RBAC, rate-limit class, idempotency, request schemas, response schemas, errors, side effects, and acceptance criteria",
  "spec-tree §20 ↔ §32.10.3.B API-contract completeness only",
  "production endpoint handlers, auth middleware, idempotency persistence, async export jobs, and runtime API tests remain product-pack evidence",
  "closes D-4.11-006 API documentation gap",
];
...
export const gate: SpecLintGate = {
  ...
  run(ctx: GateContext): Finding[] {
    ...
    requireTokens(findings, doc, sectionTextByAnchor(doc, "20.-inbox-and-pulse"), "§20 Inbox & Pulse", SECTION_20_TOKENS);
    requireTokens(findings, doc, sectionTextByAnchor(doc, "32.10.3.b-buyer-inbox-and-pulse-endpoints"), "§32.10.3.B Buyer Inbox and Pulse Endpoints", SECTION_32_TOKENS);
