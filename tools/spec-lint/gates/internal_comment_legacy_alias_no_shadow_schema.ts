/**
 * Gate: `internal_comment_legacy_alias_no_shadow_schema`
 *
 * Assertion: deprecated `/comments` routes remain only in the §32.10.9.G
 * alias map and resolve to §25.7.9 canonical Internal Comment schemas.
 */
...
const GATE_ID = "internal_comment_legacy_alias_no_shadow_schema";
const LEGACY_COMMENTS_ROUTE = /\/v1\/workspaces\/\{workspace_id\}\/comments/;

const SECTION_25_TOKENS = [
...
const ALIAS_MAP_TOKENS = [
  "The canonical Internal Comments API is §25.7.9.",
  "§32.5 now lists the canonical thread / post / mention routes instead of the pre-remediation `/comments` shorthand.",
  ...
  "Alias handling MUST emit an AuditEvent with `action='internal_comment_alias_route_used'` only if that action is registered in Appendix J before implementation.",
  "New SDKs and OpenAPI clients MUST use the canonical §25.7.9 paths only.",
] as const;
...
const ACCEPTANCE_TOKENS = [
  "Internal Comment compatibility aliases MUST resolve to §25.7.9 canonical schemas and MUST NOT introduce `/comments` shadow schemas in generated public clients.",
  "QA MUST include at least one contract test per endpoint family in this pack plus one generated-reference test that confirms legacy Internal Comment compatibility aliases appear only as deprecated alias-map entries.",
] as const;

const M5_ROW_TOKENS = [
  "Deprecated Internal Comment compatibility aliases appear only in §32.10.9.G",
  "resolve to §25.7.9 canonical Internal Comment schemas",
  "generated SDKs expose canonical `/internal-comment-*` paths only",
  "spec-tree §32.10.9.G alias-map boundary only",
  "product route shims, generated SDK output, OpenAPI generation, and runtime compatibility tests remain product-pack evidence",
] as const;

function legacyRouteEscapeFindings(doc: SpecDoc): Finding[] {
  const findings: Finding[] = [];
  for (let line = 1; line < doc.lines.length; line += 1) {
    const text = doc.lines[line] ?? "";
    if (!LEGACY_COMMENTS_ROUTE.test(text)) continue;
    const anchor = anchorForLine(doc, line);
    if (anchor !== "32.10.9.g-internal-comment-alias-binding") {
      push(
        findings,
        doc,
        line,
        "/v1/workspaces/{workspace_id}/comments",
        "Deprecated Internal Comment `/comments` route literal must appear only inside §32.10.9.G alias map.",
      );
    }
  }
  return findings;
...
export const gate: SpecLintGate = {
  ...
  run(ctx: GateContext): Finding[] {
    ...
    requireTokens(findings, doc, sectionTextByAnchor(doc, "25.7.9-internal-comments-api-surface"), "§25.7.9 Internal Comments API Surface", SECTION_25_TOKENS);
    requireTokens(findings, doc, sectionTextByAnchor(doc, "32.5-endpoints"), "§32.5 Internal Comments endpoint index", INDEX_TOKENS);
    requireTokens(findings, doc, sectionTextByAnchor(doc, "32.10.9.g-internal-comment-alias-binding"), "§32.10.9.G Internal Comment alias binding", ALIAS_MAP_TOKENS);
    ...
    findings.push(...legacyRouteEscapeFindings(doc));
