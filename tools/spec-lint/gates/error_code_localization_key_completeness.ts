/**
 * Gate: `error_code_localization_key_completeness`
 *
 * Assertion: Appendix I HTTP error-code rows carry an explicit localization
 * key of the form `error.<scope>.<code>`.
 */
...
function localizationIdx(headers: string[]): number {
  return headers.findIndex((header) =>
    header === "localization key" ||
    header === "localization_key" ||
    header.includes("localization key") ||
    header.includes("localization_key"),
  );
}

function validLocalizationKey(value: string, code: string): boolean {
  const stripped = stripMd(value);
  if (!/^error\.[a-z0-9_.]+$/.test(stripped)) return false;
  return stripped.endsWith(`.${code}`);
}

export const gate: SpecLintGate = {
  id: "error_code_localization_key_completeness",
  sourcePhase: "V8.4",
  ...
  run(ctx: GateContext): Finding[] {
    ...
    if (!appendix) {
      return [{
        ...
        message: "Appendix I is missing; localization-key completeness cannot be evaluated.",
      }];
    }
    ...
    for (let line = appendix.startLine; line <= appendix.endLine; line++) {
      ...
      const keyIdx = localizationIdx(headers);
      if (keyIdx < 0) {
        findings.push({
          file: doc.path,
          line,
          ...
          message: `${section?.heading.title ?? "Appendix I"} table must include a Localization Key column.`,
        });
        continue;
      }
      ...
      for (const row of table.rows) {
        ...
        if (!validLocalizationKey(key, code)) {
          findings.push({
            file: doc.path,
            line: row.line,
            anchor: anchorForLine(doc, row.line),
            matched_text: `${code} | ${key}`,
            message: `Appendix I row \`${code}\` must carry a localization key of form error.<scope>.${code}.`,
          });
        }
      }
      line = lastRowLine;
