...
Existing v2-cohort Buyer Free and Seller Free Orgs are eligible only when no prior accepted migration trial exists for `(org_id, console, trial_kind)`.
...
| Code | HTTP | Meaning |
| :---- | :---- | :---- |
| `solo_trial_already_redeemed` | 409 | Duplicate `(org_id, console, trial_kind)` acceptance. |
