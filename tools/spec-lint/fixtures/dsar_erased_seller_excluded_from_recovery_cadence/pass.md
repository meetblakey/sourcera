...
The enqueue→send in-flight window is protected. The dispatcher rechecks at send/retry dispatch time; dispatch MUST be dropped and recovery state left unchanged. Audit records `skip_reason='dsar_pseudonymized'` and MUST NOT persist the pseudonymized `seller_email`.
