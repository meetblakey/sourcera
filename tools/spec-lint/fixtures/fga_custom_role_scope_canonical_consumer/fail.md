...
| `seller_bid_captain`, `seller_bid_contributor`, `seller_kb_viewer`, `seller_guest` | `bid_workspace` | the Bid Workspace | §5.5 |
| `marketplace_publisher` | `organization` | the Seller Org | §5.6 |
| `marketplace_public_reader` | `organization` | the Buyer Org | §5.6 |
| `ops_*` roles | platform-wide | the Sourcera platform | §50 |
...
**`fga_custom_role_scope_type` enum.** `organization`, `workspace`, `bid_workspace`, `platform`.

#### M.5.4 Catalog index {#m-5-4-catalog-index}

| Gate ID | Source phase | Runtime status | Scope | Trigger | Failure mode | Authority anchor |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| `fga_custom_role_scope_canonical_consumer` | 3V+ | `spec_binding_pending_pack_m02_3` | Master Spec RBAC role-scope registry. | Roles map to Appendix J FGA scopes. | PR comment. | §5.13.1. |
