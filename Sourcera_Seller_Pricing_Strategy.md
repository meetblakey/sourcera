...
The per-bid option matches Seller Maya's CFO/VP-of-Sales-friendly fixed-expense mental model ("there's a deal at the end of this bid; the $199 is a known cost against that opportunity"). The subscription option is for sellers who anticipate ≥3 bids within 12 months. The two paths share one entitlement set; a seller can convert mid-bid under Master Spec §34.2.5, which owns annual-invoice application and monthly residual-credit rollover.
...
- **Enterprise controls** (SSO/SCIM/residency, custom DPA negotiation, custom RevOps integrations). Standard DPA access and the role-gated Audit Event Export API remain all-plan under Master Spec §34.1.2 / §32.8.24.
...
- **Mid-bid conversion.** Master Spec §34.2.5 owns the non-cash conversion credit: annual cadence applies it to the conversion invoice; monthly cadence rolls any residual across later Solo invoices. It is not refunded or transferable.
...
- Paid commitments are not purchasable in v7.x. Master Spec §34.16.3 reserves the feature behind an off-by-default flag; any future activation follows that section's binding approval and disclosure contract.

---

## 14. Financial Scenarios

*(Was §13 in v2; renumbered and extended for v3 with a Solo scenario SPS-D.)*

Same assumptions as buyer-side: Anthropic mid-2025 pricing, infrastructure-overhead reserve follows Master Spec §34.14.1.c, CSM loaded only on Scale/Enterprise.

### Scenario SPS-A — 200 Seller Starter customers
...
### Scenario SPS-D — Seller Solo tier mixed cohort *(new in v3)*
...
Operational definitions, cohort denominators, cadence, and breach routing live in Master Spec §48.1.6 and §34.18.5. This list is narrative priority only and does not authorize alternate events or thresholds.
...
**Follow-up closure status (current Master Spec authority):**

- Master Spec §44.6 now owns Solo-Tier Surface Treatment, including engine-absorbed consumption and silent throttling.
- Master Spec §22 / §22.20 owns Seller KB behavior and Seller surface compression; plan numerics remain single-sourced in §34.1.2.
- Master Spec §21.4 / §21.4.5 owns capability registration, Solo routing, and outcome signals.
- Master Spec §4.4.21 / §27.11.3 owns Verification review behavior and surfaces; §34.1.2 owns Seller Solo eligibility.
- Master Spec §5.11 owns role/feature access and cites §34.1 for plan-tier authority.
- Master Spec §21.4 / §22 / §34 is the current capability-registry, Seller-KB, entitlement, and pricing chain. No Phase 14.9.1 / 14.10 item in this block remains pending.
