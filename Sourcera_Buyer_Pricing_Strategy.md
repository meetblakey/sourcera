...
**v3 changeset:** Added the **Buyer Solo tier** ($49/mo annual / $59/mo monthly OR $199/eval one-time) as the personal-card on-ramp between Free and Business Starter — the surface for **Buyer Maya**, the first-time evaluation owner introduced as the v7.1.0 buyer-side surface persona. AI consumption is invisible to Solo-tier users (cost absorbed within the Master Spec §34 / §44 Solo value envelope; engine throttles silently if approached). Solo unlocks the unwatermarked Selection Report and the new **Defense View** (Master Spec §13.11). Free now ships with a watermarked Selection Report and a preview-only Defense View. Updated plan-architecture table, §1 executive summary, PLG paid path (now Free → Solo → Starter → Growth → Scale → Enterprise), Financial Scenarios (added Solo scenario BPS-D), and Migration delta. **Phase 14.9 — Master Spec scaffolding edit landed (2026-04-27):** all numerical limits, charge orchestration, plan tier enum values, surface/engine mappings, and Stripe meter events are now authoritative in `Sourcera_Master_Spec.md` §34.1.1 (Buyer Plan Tiers), §34.2.1 (Buyer Pricing), §34.2.5 (Solo Per-Evaluation Charge Orchestration), §34.10.3 (Solo-co-resident pool rule), §34.12.6 (Solo per-console subscription billing), §39 (Object Size Constraints — Solo cells cite §34.1.1 and never restate), Appendix J (Plan Tiers — `buyer_solo` registered; `billing_event_charge_kind` extended with `solo_per_eval`), and Appendix M (Surface/Engine Mapping — Solo billing surface, Solo Defense View, Solo silent throttling, Solo Mode pipeline compression rows). This document remains the narrative-and-strategy companion; numerical authority is in the Master Spec.
...
Value price for any AI operation = `cost_base × 10`. Rejected-op price = `cost_base × 1.05`. When Anthropic pricing moves, the multiplier holds and the underlying cost_base updates. Included-budget plans are sized so that the worst realistic accept/reject mix still clears 88% margin before CSM and Convex compute. **Solo's absorbed envelope is sized from the Master Spec §34 authority** — the engine guarantee carries; the surface just hides it. The throttling logic protects margin without exposing the operator to bill-shock or rate-card complexity.
...
| Included AI budget (value $) | $5/mo | **Hidden (§34.1.1 / §34.2.5)** | $50/mo | $300/mo | $800/mo | Committed |
...
| Integrations | Core only | Core + standard Phase-13 export | Core + standard export | Core + standard export | Core + standard export | Core + standard export + custom adapters |
| SSO / SCIM | — | — | — | — | — | ✅ |
| IP allowlist / data residency | — | — | — | — | — | ✅ |
| Audit export / DPA | Included / Standard | Included / Standard | Included / Standard | Included / Standard | Included / Standard | Included / Standard + custom DPA by contract |
...
**New-buyer trial.** Every new Buyer Console Org starts on the 14-day Business Starter trial without a credit card. Day 15 converts to the selected paid tier when a payment method is on file; otherwise the Org automatically moves to Buyer Free with the §34.6 preservation path. This is an onboarding state, not a separate plan. The full contract is Master Spec §34.9.1.
...
The per-evaluation option matches Buyer Maya's CFO-friendly fixed-expense mental model. The subscription option is for operators who anticipate ≥3 evaluations within 12 months. The two paths share one entitlement set; an operator can convert mid-evaluation under Master Spec §34.2.5, which owns annual-invoice application and monthly residual-credit rollover.
...
- **AI consumption invisible.** Cost absorbed against the Master Spec §34.1.1 / §34.2.5 Solo value envelope. Engine throttles low-priority background capabilities silently if envelope is approached. Operator never sees value-dollars, wallet UI, rate card, or per-op pricing.
...
- **Enterprise controls** (SSO/SCIM/residency and custom DPA negotiation). Standard DPA access and the role-gated Audit Event Export API remain all-plan under Master Spec §34.1.1 / §32.8.24.
...
- **Mid-eval conversion.** Master Spec §34.2.5 owns the non-cash conversion credit: annual cadence applies it to the conversion invoice; monthly cadence rolls any residual across later Solo invoices. It is not refunded or transferable.
...
Operational definitions, cohort denominators, cadence, and breach routing live in Master Spec §34.18.5. This list is narrative priority only and does not authorize alternate events or thresholds.
...
- **Solo's absorbed envelope is denominated in value-dollars and is reviewed with every cost-base recalculation.** Anthropic price movement does not silently mutate a live threshold. Ops Finance reviews the §44.6 throttle-event rate and may publish a new per-capability threshold through the audited Capability Registry / PricingTableVersion path; the operator's fixed Solo price holds. Master Spec §44.6.4 and §34.3.3 own the mechanism.
...
*(Was §12 in v2; renumbered and extended for v3 with a Solo scenario BPS-D.)*

Assumptions: Anthropic mid-2025 pricing; infrastructure-overhead reserve follows Master Spec §34.14.1.c; CSM loaded only on Scale (shared) and Enterprise (dedicated).

### Scenario BPS-A — 100 Starter customers
...
### Scenario BPS-D — Solo tier mixed cohort *(new in v3)*
...
- AI COGS:
  - Subscription: 200 × $0.54/mo absorbed (full §34 Solo envelope at 70/30 mix) × 12 = $1,296
...
- Master Spec §34.1.1 (Buyer Plan Tiers) — Buyer Solo column registered with all gating numerics; pricing docs cite §34.1.1, never restate.
- Master Spec §34.2.1 (Buyer Pricing) — Solo row added with subscription ($49 annual / $59 monthly) and per-eval ($199) pricing.
- Master Spec §34.2.5 (Solo-Tier Per-Evaluation / Per-Bid Charge Orchestration) — newly authored authoritative section.
- Master Spec §34.10.3 (Pooled Budget Across Consoles) — Solo-co-resident pool rule added; Solo's engine-absorbed envelope is never customer-visible.
...
- Master Spec §34.12.1 (Per-Console Plan Activation) — Solo combinations added.
- Master Spec §34.12.6 (Solo-Tier Per-Console Subscription Billing) — newly authored authoritative section.
- Master Spec §34.12.8 (Cross-Side Billing Failure Modes) — Solo failure modes added (Failure Modes #5–#7).
- Master Spec §39 (Object Size Constraints) — Solo plan-tier object-size constraints cite §34.1.1 cells; Solo header note added.
- Master Spec §5.11 (Feature Access Matrix) — Solo plan-tier registration note added; existing Solo Mode default surface note preserved.
- Master Spec Appendix J — Plan Tiers enum split into per-console `buyer_plan_tier` / `seller_plan_tier` with `buyer_solo` / `seller_solo` registered; `billing_event_charge_kind` enum added (`solo_per_eval`, `solo_per_bid`, `solo_charge_refunded`).
- Master Spec Appendix M — Buyer / Seller Plan Tier enum rows updated; Solo billing surface, Solo engine-absorbed envelope, Solo per-eval / per-bid orchestration, Solo Selection Report watermarking, Solo Defense View access, Solo Verified-tier eligibility, Solo Capability Declaration auto-publish, Solo Mode pipeline compression, and Solo upgrade CTA rows added; Pro Trial Seat row's Hidden-from-tier list extended to include `Bs (Buyer Solo)`; tier-abbreviation legend updated to register `Bs` and `sSo`.

**Spec sections still pending (Phase 14.9.1 / 14.10 follow-up):**
- Master Spec §44 (Solo-Tier Surface Treatment — engine-only AI consumption + silent throttling) — Phase 14.10 deliverable.
- Master Spec §21.4 / §21.4.5 (capability catalog) — `defense_view_generate` capability registration with `surface_throttling_class = active_workflow` (Phase 14.10).
- Master Spec §5.11 inline plan-tier list audit (lines 8714, 8858, 8926, 8991, 9062, 11766, 14952, 18474, 18943, 19052, 19641, 20542, 23915, 24392, 27113, 28077, 28350, 28463, 28669, 29830, 29956, 30120, 30565, 33402, 33437, 39292, 41435, 41576, 44566, 45938) — Phase 14.9.1 follow-up to verify each inline tier-list string against §34.1.1 / §34.1.2 cell values.
