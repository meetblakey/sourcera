...
If the canonical `Requirement` label has the wrong native scope, use the separate Requirement-label replacement workflow. Each main-only dispatch performs one receipt-chained phase: plan, rename, create the preallocated REQ-scoped replacement, replace the pinned assignments serially, verify, retire, or finalize two stable captures. Every mutation phase supports digest-pinned crash recovery; compensation reverses receipt-proven work without deleting labels and preserves any unavoidable retired residual under a unique name. Authenticated captures are read-only, while candidate construction and verification run without the Linear credential. A later main commit is accepted only when it descends from the plan commit and every migration control remains byte-identical. The authority baseline accepts only the pinned finalize artifact and exact new UUID, then rechecks the retired old UUID has zero uses and the new UUID has exactly the 186 pinned REQ uses. It has no legacy or discovery fallback. Both workflows share one non-canceling write lock.
