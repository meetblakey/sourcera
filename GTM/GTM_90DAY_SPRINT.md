...
**Source authority:** `Sourcera_Master_Spec.md` §1, §2, §27, §34, §48.0.1, §48, §49 · `GTM_POSITIONING.md` v1.0 · `GTM_PLG_ARCHITECTURE.md` v1.0 · `GTM_SALES_PLAYBOOK.md` v1.0 · `GTM_CONTENT_ENGINE.md` v1.0.
...
**How to read this document.** Hours are the wall-clock time the founder spends — deep work, demos, writing, CRM hygiene, all inclusive. The ceiling is 50 hrs/wk; most weeks land between 44 and 50. Every week explicitly covers both sides of the marketplace (Buyer Console and Seller Console) because the Dual-Console architecture in Master Spec §1 / §7 is the product's structural core and the flywheel (`GTM_SALES_PLAYBOOK.md` §1.3) only compounds when both sides ship simultaneously. Where hours for a week sum below 50, the residual is the explicit Float block reserved for opportunistic writes, partner calls, and contingency — not padding.

**Drift anchors.** Every reference below is load-bearing against the authoritative category name — *Software Evaluation Platform* — the 13-phase pipeline, the Dual-Console firewall, outcome-accounted AI pricing, unlimited seats, and the §48.0.1 commercial wedge: buyer evaluation creates seller urgency; a drafted bid creates fast seller value; seller supply compounds into Marketplace and SEO value. No section uses adjacent-category softening ("procurement suite," "RFP tool," "AI for procurement"). Growth Mechanics are named by M-number; the launch set at GA is **M1, M2, M4, M5, M8, M14, M15, M17** (`GTM_PLG_ARCHITECTURE.md` §5.9). M3, M6, M7, M9, M10, M16 ship in Month 2. M11, M13 ship Month 3–4. M12, M18+ ship Month 6–9. This plan does not pretend to launch M9/M10/M11 at GA — doing so silently is the single most common pre-launch drift pattern.
...
|---|---|---|---|
| 1.1.1 | Buyer Free workspace creation, Cmd+K, Linear-benchmark shell | Workspace creatable in < 60 s from magic-link; Cmd+K navigates; 8 UX principles pass (Master Spec §3) | Master Spec §3 / §11 / §34.1.1 |
| 1.1.2 | Buyer signup — WorkOS SSO + email magic-link | 7-step signup < 2 min p50; WorkOS OSS/OIDC path works on 2 test IdPs | Master Spec §6.1.1; `GTM_PLG_ARCHITECTURE.md` §2 |
| 1.1.3 | Template Library seeded — 19 curated templates | All 19 templates imported, clone-into-workspace works, categorized: CRM/ERP/Security/ITSM/HRMS/DevOps/Data/AI-Infra/Finance | Master Spec §19; `GTM_PLG_ARCHITECTURE.md` §2.3 |
| 1.1.4 | 13-phase pipeline — Phases 1–12 operational; Phase 13 (post-award) stubbed with copy | Buyer can walk a test evaluation end-to-end; Phase 13 shows a "coming soon" pane citing Master Spec §10 | Master Spec §10 |
| 1.1.5 | Pre-Scoring capability working across 3 sample vendors | Aha Moment fires; Pre-Scoring Report renders; PostHog event `buyer_prescoring_completed` emits | `GTM_PLG_ARCHITECTURE.md` §2.4; Master Spec §13 / §21 / §51 |
| 1.1.6 | FM/PM/EJ Scoring Engine with Disagreement Insight Cards | Scoring runs across ≥10 requirements, 2 reviewers, surfaces disagreement ≥0.3 | Master Spec §13.5 / §13.6 |
| 1.1.7 | Selection Report generator (Phase 12) | DOCX + PDF export; SHA-256 hash emitted; matches the template shipped as Asset 8 | Master Spec §4.3.23 / §4.3.24 / §10.12 |
| 1.1.8 | Buyer AI Budget Meter — real-time consumption, $5/$50/$300/$800 | Meter renders on all Console screens; 80% warning email via Loops at threshold | Master Spec §34; `Sourcera_Buyer_Pricing_Strategy.md` §2.4 |
| 1.1.9 | Policy-Powered Requirement Generation — NIST 800-53 Rev 5 test upload | Upload parses, dedupes at ≥90% confidence, produces ≥80 scorable requirements | Master Spec §12 |
| 1.1.10 | TCO Modeling (6 pricing schemas) | Flat/tiered/one-time/%-of-license/estimate-range/discount all working; 1/3/5-year projections render | Master Spec §15 |
| 1.1.11 | Audit Log visible to buyer admins | Append-only, SHA-256 chained, ≥24 event types emitting | Master Spec §6.7 / §4.6.1 |

### 1.2 Product Surface — Seller Console

| # | Item | Acceptance | Source |
|---|---|---|---|
| 1.2.1 | Seller Hero Moment — 0-3 min onboarding from magic-link | p50 < 20 min magic-link → first submitted response across 5 dogfood bids | Master Spec §48.0.1 / §48.1.6 / §48.8 |
| 1.2.2 | Seller Free — published profile on signup with domain verification | Profile publishes within 60 s of email-verified login; Schema.org Organization/SoftwareApplication emitted | Master Spec §26.1 / §26.8 / §26.9.1 |
| 1.2.3 | KB core — 50-entry Free cap; entry lifecycle; cite-before-state default | Free cap enforces; cap-hit event fires; Haiku draft mode works without Opus fallback at Free tier | Master Spec §22 / §22.12 / §22.16.1 / §34.1.2 |
| 1.2.4 | Bid Workspace — First-Pass Responder on invited bid | Bid draft generated from KB + Requirements within 3 min; confidence scores render | Master Spec §22.14.1 / §24 |
| 1.2.5 | Ghost-Bid Importer (M15) | 12-month-old RFP import produces ≥40 approved KB entries; provenance captured | Master Spec §4.4.17 / §22.14.4 / §48.7.4 |
| 1.2.6 | Seller AI Budget Meter — $5/$30/$200/$600 | Meter renders; budget-ceiling behavior aligns with seller pricing authority | `Sourcera_Seller_Pricing_Strategy.md` §2.2; Master Spec §34.3 |
| 1.2.7 | Capability Declarations — author + attach to SellerSoftware entity | Can declare ≥3 capabilities; appears in category-matched Marketplace inventory | Master Spec §4.4.4 / §26.3 / §26.8.3 / §27 |
| 1.2.8 | EOI workflow stub — locked until buyer-side shortlist | Screen shows EOI gated; copy explains Starter unlock (conversion moment #2) | `GTM_PLG_ARCHITECTURE.md` §4.2; Master Spec §27.5 / §34.1.2 |
| 1.2.9 | Bid Success Share (M14) | Win/loss outcome capture emits anti-metric to seller dashboard | M14 |
| 1.2.10 | Pro Trial Seat provisioning (M17) for Scale buyers inviting sellers | 5 trial seats/mo on Scale; fulfillment runs on M1 invite path | Master Spec §34.13 / §48.7.4 |
...
| 1.4.4 | Loops.so lifecycle events wired — 8 event triggers minimum (`GTM_CONTENT_ENGINE.md` Section 4.4) | `buyer_free_signup`, `seller_forced_signup_day0`, `seller_free_day7`, `seller_free_day30`, `seller_concurrent_bid_limit_hit`, `buyer_ai_budget_80pct`, `kb_bootstrap_complete`, `selection_report_generated` all firing |
...
| # | Item | Acceptance | Source |
|---|---|---|---|
| 1.5.1 | Marketing site — Framer, 7 core pages | Home, /method, /pricing (buyer), /pricing-sellers, /for-buyers, /for-sellers, /about | `GTM_POSITIONING.md` §7 |
| 1.5.2 | The Sourcera Method cornerstone published at `/method` | 10K-word guide; `HowTo` + `Article` schema; 13-chapter IA | `GTM_CONTENT_ENGINE.md` §2.4 |
| 1.5.3 | Asset 1 — Method + Templates Bundle (PDF + 8 templates) | Email-gated; DOCX + PDF + GDoc versions of all 8 templates | `GTM_CONTENT_ENGINE.md` §5.1 |
| 1.5.4 | Substack *The Evaluation* + LinkedIn newsletter mirrored | Issue 1 drafted and scheduled for T+0 | `GTM_CONTENT_ENGINE.md` §4 |
| 1.5.5 | Loops nurture sequences — 6 drips live | Buyer Day 0/3/7/14; Seller Day 0/7/30; newsletter welcome | `GTM_CONTENT_ENGINE.md` §4.4 |
| 1.5.6 | Founder LinkedIn profile + About block + featured section | About block names the category; featured = Method post + Asset 1 download | `GTM_CONTENT_ENGINE.md` §3 |
| 1.5.7 | Attio CRM — 4 pipelines live | Buyer Outbound, Buyer PQL, Seller Outbound, Seller PQL | `GTM_SALES_PLAYBOOK.md` §1.5 |
| 1.5.8 | Instantly.ai — 3 inboxes on `sourcera.io` domain, warmed | 14-day warmup complete by T-0; avg deliverability ≥95% | `GTM_SALES_PLAYBOOK.md` §1.5 |
| 1.5.9 | 20 blog posts outlined — Posts 1–3 fully drafted | Post 1 = Method cornerstone; Posts 2–3 queued for W2–W3 | `GTM_CONTENT_ENGINE.md` §2.3 |
| 1.5.10 | Demo deck + product-walkthrough Loom (buyer) + (seller) | Two 2-min Looms, two 8-min structured demos, all from dogfood workspace | `GTM_SALES_PLAYBOOK.md` §6 |
...
| # | Item | Acceptance |
|---|---|---|
| 1.7.1 | Founder Dashboard (Notion + PostHog embeds) — 14 leading indicators | All 14 metrics (`GTM_SALES_PLAYBOOK.md` §1.4) rendering daily |
...
| Content | Draft Post 1 — *The Sourcera Method* cornerstone, Sections 1–6 (first half of 10K words) | 5 | Cross | `GTM_CONTENT_ENGINE.md` Section 2.4 |
| Content | LinkedIn profile rework — headline, About, Featured, banner. Post first founder-journal *"I'm building the operating system for enterprise software procurement"* | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 3 |
...
| Block | Task | Hours | Side | Tie-back |
|---|---|---|---|---|
| Content | Post 1 — complete Sections 7–13 + on-page SEO (title, meta, URL=`/method`, internal links, `HowTo`+`Article` schema). Publish preview to Framer | 6 | Cross | `GTM_CONTENT_ENGINE.md` Section 2.4 |
| Content | Asset 1 — package Method as 70-page PDF + DOCX/GDoc of the 8 templates; Canva cover art | 8 | Cross | `GTM_CONTENT_ENGINE.md` Section 5.1 |
...
| Content | LinkedIn posts #1, #2 from Section 3.2 catalog — "Every vendor evaluation produces one honest artifact" (Post 1) and "I uploaded NIST 800-53 to Sourcera…" (Post 2 teardown) | 1.5 | Buyer | `GTM_CONTENT_ENGINE.md` §3.2 |
| Product | Convex perf — 50 RPS load test against Console, fix top-3 p95 regressions | 6 | Cross | 1.4.2 |
| Product | Loops.so — wire 8 lifecycle event hooks to PostHog; test all 8 drips to dummy account | 5 | Cross | 1.4.4, `GTM_CONTENT_ENGINE.md` Section 4.4 |
...
| Sales ops | Write cold-email Sequence A (Priya) — 4 emails — tuned for 2026-05 send; schedule through Instantly starting T+0 | 1.5 | Buyer | `GTM_SALES_PLAYBOOK.md` §2.1.2 |
| Sales ops | Write cold-email Sequence C (Seller Organic) — RFP-response-efficiency angle — 4 emails | 1.5 | Seller | `GTM_SALES_PLAYBOOK.md` §3 |
...
| Product | Selection Report PDF + DOCX export polish; SHA-256 chain audit; run on 3 dogfood evals | 3 | Buyer | Master Spec §4.3.23 / §4.3.24 / §10.12, 1.1.7 |
| Product | KB 50-entry cap enforcement; conversion-moment event `seller_kb_50_cap_hit` wired to Loops | 2 | Seller | `GTM_PLG_ARCHITECTURE.md` §4.2 |
...
| Content | Draft Newsletter Issue 1 ("Announcing *The Evaluation*" + Method preview, 1,100 words) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts #7 ("G2 vs. Gartner vs. your actual procurement process" carousel, 6 slides) + #11 ("Your RFP response takes 2 days") | 1.5 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 2 prep — draft category evaluation templates for **Security/GRC** and **Data/Analytics** (2 of 6 seed categories) | 4 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 2 |
| Marketing | /for-buyers and /for-sellers pages complete with persona-specific copy from `GTM_POSITIONING.md` §5 | 3 | Cross | 1.5.1 |
| Marketing | Status page + legal pages live; DSAR workflow filed in `/ops/dsar.md` | 2 | Cross | 1.4.7, 1.6.1–1.6.3 |
| Sales ops | Persona cadences: Sequence B (Marcus VP IT), Sequence D (Elena CISO) drafted and loaded into Instantly | 2 | Buyer | `GTM_SALES_PLAYBOOK.md` §2.1.2 |
| Sales ops | Write seller magic-link invite email copy + second-invite follow-up (for sellers who land, don't engage) | 1.5 | Seller | Master Spec §48 |
| Sales ops | Partner outreach — 3 warm intros: Pavilion procurement channel, PreSales Collective RFP community, ProcurementFoundation Slack admin (no launch pitch, relationship-only) | 2 | Cross | `GTM_SALES_PLAYBOOK.md` §4 |
...
| Content | Post 1 — flip to `/method` live URL (soft, no broadcast); internal link pass across posts 1, 2, 3 | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 2.4 |
| Content | Post 2 and Post 3 scheduled for W2 and W3 publication respectively | 1 | Cross | Posts 2, 3 |
| Content | Newsletter Issue 1 scheduled for Monday 2026-05-18 07:00 ET | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts #9 (founder journal — *"I'm building Sourcera. Here's where we lost a deal last week"*) — anonymized dogfood loss | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 1 download page + gate (ConvertKit/Loops form); landing-page copy in Framer | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 5.1 |
...
| Sales ops | Cold-email Sequence A (Priya) pre-warm — 40 sends from Instantly Tuesday/Wednesday before launch | 1 | Buyer | `GTM_SALES_PLAYBOOK.md` §2.1.2 |
| Demo | Two live rehearsal demos — one buyer, one seller — with friendly advisor as critic | 2 | Cross | `GTM_SALES_PLAYBOOK.md` §5, §6 |
...
| Press/PR | 5 targeted journalist/analyst intros (Spend Matters editor, Pavilion, ProcurementFoundation, SaaStr, Lenny) with Method PDF attached — pre-embargo | 3 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.2 |
| Community | Pavilion #procurement intro post (not promotional; contribution to an active thread) | 1 | Buyer | `GTM_CONTENT_ENGINE.md` Section 3.4 |
| Community | PreSales Collective intro post — KB governance take | 1 | Seller | `GTM_CONTENT_ENGINE.md` Section 3.4 |
...
| Thu | 09:00 | Win/loss debrief (`GTM_SALES_PLAYBOOK.md` §1.2) — any deal closed or stalled in Days 1–3 gets a 1-page writeup | Notion `/ops/wl/` | 1 |
...
| Fri | 10:00 | ICP drift review — Fri-afternoon per `GTM_SALES_PLAYBOOK.md` §1.2; which signups fit ICP, which don't, log drift | Notion | 1 |
...
| Block | Task | Hours | Side | Tie-back |
|---|---|---|---|---|
| Product | PQL model calibration — re-weight the seven signal components from real Week-1 data (signup velocity, Aha-Moment fire rate, AI budget consumption, vendor invite count, Policy Upload, Selection Report generation, Pro Trial claim) | 6 | Buyer | `GTM_PLG_ARCHITECTURE.md` §7 |
| Product | Seller PQL model calibration — weight components (Hero Moment completion, Ghost-Bid Importer usage, second-invite arrival, EOI attempt, KB entry velocity) | 4 | Seller | `GTM_PLG_ARCHITECTURE.md` §7 |
...
| Content | Newsletter Issue 2 — Method Chapter 1 (the 13 phases), 1,100 words | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #3 (scoring carousel, 7 slides), #7 (G2/Gartner/process carousel), #10 (SOC 2 audit findings) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 2 — ship **Security/GRC** and **Data/Analytics** evaluation templates (2 of 6 seed categories) via same gated-download pattern | 4 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 2 |
| Sales (buyer) | Daily PQL triage — first-touch every buyer PQL ≥50 within 24h; deeper touch on ≥100 same day | 6 | Buyer | `GTM_SALES_PLAYBOOK.md` §1.2 |
| Sales (buyer) | Demo week — target 5 buyer demos | 5 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (seller) | Second-invite outreach — every seller PQL ≥50 that arrived W1; founder-written 3-line email referencing their specific bid | 3 | Seller | Master Spec §48 |
| Sales (seller) | Target 3 seller demos (Growth tier candidates — ≥3 concurrent bids already) | 3 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
| Ops | Stripe reconciliation; CRM hygiene; Win/Loss debrief for stalled Week-1 deals | 2 | Cross | `GTM_SALES_PLAYBOOK.md` §1.2 |
...
| Product | KB Health Score (Master Spec §22.5) surfacing on Seller dashboard — Loops drip when Score declines | 3 | Seller | Master Spec §22.5 |
| Product | Seller Bid Workspace — cite-before-state enforcement; any AI-generated claim without citation is flagged | 4 | Seller | Master Spec §22.12 / §22.16.1 / §24 |
...
| Content | Newsletter Issue 3 — Scoring as the core IP (Method Chapter 4) | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #12 (KB 8-piece carousel), #14 (proposal-manager single-point-of-failure), #13 (Ghost-Bid teardown) | 2 | Seller | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 5 draft — RFP Response Reuse Rate Calculator (seller-side flagship, logic + React page) | 6 | Seller | `GTM_CONTENT_ENGINE.md` §5.1 Asset 5 |
| Sales (buyer) | Daily PQL triage + 4 demos + cold outbound Sequence A Email 3 (rolling cohort) | 12 | Buyer | `GTM_SALES_PLAYBOOK.md` §2 |
| Sales (seller) | 5 seller demos; second-invite outreach; PreSales Collective monthly teaching post — "Why KB governance beats content libraries" | 5 | Seller | `GTM_CONTENT_ENGINE.md` Section 3.4 |
| Ops | Weekly review; Win/loss; ICP drift | 2 | Cross | `GTM_SALES_PLAYBOOK.md` §1.2 |
| Partner | Intro call with Pavilion procurement channel lead — offer *Evaluation* newsletter swap + monthly AMA | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.3 |
| Float | Contingency | 3 | — | — |
| **Total** | | **50** | | |

### Week 4 (June 8–12) — First Upgrade Wave, Buyer Cohort Depth

The 3-week post-launch window is when buyer Free-to-Starter conversion first materializes. Focus founder attention on the in-product upgrade nudge (`GTM_PLG_ARCHITECTURE.md` §4.1 nudges 1–5) and the conversion-trigger emails.

| Block | Task | Hours | Side | Tie-back |
|---|---|---|---|---|
| Product | Free-to-Starter upgrade surface — 5 in-product nudges (`GTM_PLG_ARCHITECTURE.md` §4.1) polished: AI budget 80%, concurrent-eval attempt, vendor invite beyond Free cap, Policy Upload beyond Free limit, Enterprise-feature preview | 5 | Buyer | `GTM_PLG_ARCHITECTURE.md` §4.1 |
...
| Content | Newsletter Issue 4 — Audit-defensibility essay (P2 pillar) | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #5 (annotated Selection Memo teardown), #4 (seat-tax argument), #17 (proposal-manager SPOF) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Post 10 publishes Thursday — *"The RFP Response OS"* (seller cornerstone) | 0.5 | Seller | Post 10 |
| Content | Asset 5 ship — RFP Response Reuse Rate Calculator live + gated report + nurture drip | 4 | Seller | `GTM_CONTENT_ENGINE.md` §5.1 Asset 5 |
| Content | Guest essay pitch round 2 — Dark Reading (compliance-to-requirement angle), SaaStr (unlimited-seats decision angle) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.2 |
| Sales (buyer) | 6 buyer demos; Starter upgrade conversations for each PQL ≥100 in the cohort | 10 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (buyer) | Cold outbound — Sequence B (Marcus) launch; Sequence D (Elena) launch; 3 inboxes rotating | 2 | Buyer | `GTM_SALES_PLAYBOOK.md` §2 |
| Sales (seller) | 4 seller demos; post-second-concurrent-bid outreach (conversion moment #1) | 4 | Seller | `GTM_PLG_ARCHITECTURE.md` §4.2 |
| Sales (seller) | Responsive/Loopio direct outbound — 20 targeted reach-outs per `GTM_SALES_PLAYBOOK.md` §3 outbound list | 2 | Seller | `GTM_SALES_PLAYBOOK.md` §3 |
| Ops | Weekly review; Win/loss; Stripe reconciliation; first revenue recognition run | 2 | Cross | — |
| Podcast | First recorded appearance — *PreSales Collective Podcast* (booked from W-1 pitch) | 2 | Seller | `GTM_CONTENT_ENGINE.md` Section 6.3 |
| Float | Contingency | 3 | — | — |
| **Total** | | **50** | | |
...
| Block | Task | Hours | Side | Tie-back |
|---|---|---|---|---|
| Content | LinkedIn Newsletter *The Evaluation* launches — Issue 1 mirror + founder-journal framing | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.5 |
...
| Content | Newsletter Issue 5 — Policy-to-Requirement essay | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #2 (NIST teardown deeper cut), #18 (Apollo vs. Sourcera seller angle), #6 (scoring variance 0.3 pt) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 2 expansion — ship 2 more seed-category templates (DevOps and RevOps); 4 of 6 done | 4 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 2 |
| Partner | Newsletter swap execution — feature ProcurementFoundation in Issue 5; they feature *The Evaluation* in their weekly | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.3 |
| Partner | Pavilion monthly AMA scheduled for Week 6; prep FAQ | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.4 |
| Sales (buyer) | 6 demos; 2 Growth upgrade conversations (month-3 upgrade target) | 10 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (buyer) | Cold outbound volume now at steady-state 120 sends/day across 3 inboxes | 3 | Buyer | `GTM_SALES_PLAYBOOK.md` §2 |
| Sales (seller) | 5 demos; EOI-attempt outreach (conversion moment #2) on any seller whose EOI click was blocked | 5 | Seller | `GTM_PLG_ARCHITECTURE.md` §4.2 |
| Sales (seller) | Seller Starter upgrade debrief — why the first 3–5 seller Starters converted; what the 10 non-converters had in common | 2 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
...
|---|---|---|
| Buyer Free signups | 60 | `GTM_SALES_PLAYBOOK.md` §1.4 Months 1–3: 40/mo |
| Seller signups (combined Forced + Organic) | 120 | `GTM_SALES_PLAYBOOK.md` §1.4: 60/mo |
| Hero Moment p50 | ≤20 min | Master Spec §48.1.6 |
| Active evaluations | 30 | 5 North Stars |
| Selection Reports generated | 6 | — |
| Buyer paid conversions | 1–3 | `GTM_SALES_PLAYBOOK.md` §1.4: 1/mo baseline |
| Seller paid conversions | 1–3 | `GTM_SALES_PLAYBOOK.md` §1.4: 1/mo baseline |
| Newsletter subscribers | 700 | `GTM_CONTENT_ENGINE.md` Section 4.5 Month-1 target 200 × updated trajectory |
| LinkedIn followers | 1,100 | — |
| Blog posts live | 5 (Posts 1, 2, 3, 4, 5) | `GTM_CONTENT_ENGINE.md` Section 7.3 |
| Gated assets live | 3 (Asset 1, 2 partial, 5) | `GTM_CONTENT_ENGINE.md` Section 5.1 |
| MRR | $1,200–$3,600 | `GTM_SALES_PLAYBOOK.md` §1.4 |

---

## SECTION 5 — POST-LAUNCH MONTH 2 (Weeks 6–9: June 22–July 17)

The Month-2 thesis is **expand the M-number set and activate programmatic SEO**. Month 2 ships M3, M6, M7, M9, M10, and M16 (`GTM_PLG_ARCHITECTURE.md` §5.9 "Month 2" set). Content production scales from 1 to 2 long-form posts per month at steady cadence. The first M9 category pages (Security/GRC, Data/Analytics) go live.
...
| Product | First 2 M9 category pages live — Security/GRC, Data/Analytics (matches Asset 2 ship order) | 3 | Cross | M9; `GTM_POSITIONING.md` §1.3.1 |
...
| Content | Newsletter Issue 6 — *The RFP Response OS* reprise | 2 | Seller | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #11 (KB carousel), #16 (Hero Moment journal), #19 (KB 47→183 teardown) | 2 | Seller | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Partner | Pavilion monthly AMA — live, 60 min | 1 | Buyer | `GTM_CONTENT_ENGINE.md` Section 6.4 |
| Sales (buyer) | 7 demos; 2 Growth upgrade conversations; 1 Scale conversation if any PQL ≥150 | 12 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (seller) | 5 demos; second-invite outreach on Week-4/5 cohort | 5 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
| Sales | Cold outbound steady-state; new seller Sequence (Growth/Scale tier — 20 target accounts/wk with ≥100 employees in Presales-heavy industries) | 3 | Seller | `GTM_SALES_PLAYBOOK.md` §3 |
| Ops | Weekly review; Win/loss | 1.5 | Cross | — |
| Podcast | Second podcast recording — *Art of Procurement* (pitched W-1, booked W4) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.3 |
| Float | Contingency | 5 | — | — |
| **Total** | | **50** | | |
...
| Content | Newsletter Issue 7 — *"The $5 AI budget as a product decision"* (pricing narrative) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #4 (seat-tax argument), #20 (Sourcera Method as published IP), #15 (FM/PM/EJ carousel) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 2 — ship remaining 2 seed-category templates (HRIS, AI Infrastructure); all 6 of 6 done | 4 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 2 |
| Sales (buyer) | 7 demos; Scale negotiation (first Scale candidate by end of Month 2) | 12 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (seller) | 5 demos; Starter → Growth upgrade conversations on sellers completing their 3rd concurrent bid | 5 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
...
| Product | Enterprise gate polish (Master Spec §34) — SSO requirement, DPA, custom integration trigger → founder-routed workflow | 3 | Cross | Master Spec §34 |
...
| Content | Newsletter Issue 8 — *"The Hero Moment"* (seller-deep issue) | 2 | Seller | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #16 (Hero Moment narrative), #12 (RFP OS carousel encore), #8 (40-hour healthcare-buyer journal) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Guest essay #1 ships — Spend Matters — *"A New Category: Software Evaluation Platform"* | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.2 |
| Sales (buyer) | 8 demos — larger cohort from M9/M10 SEO-sourced traffic hitting homepage | 12 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (buyer) | First Enterprise pitch — SOC 2 + DPA + ≥$12K committed AI gate all met; 90-min structured demo + proposal | 3 | Buyer | `GTM_SALES_PLAYBOOK.md` §8 |
| Sales (seller) | 5 demos; EOI-attempt outreach on Starter sellers whose EOI click was blocked in prior 14 days | 5 | Seller | `GTM_PLG_ARCHITECTURE.md` §4.2 |
| Sales | Cold outbound SEQUENCE expansion — add Sequence E (Dan CFO) for Scale candidates | 2 | Buyer | `GTM_SALES_PLAYBOOK.md` §2 |
| Ops | Weekly review; Win/loss; refresh PQL thresholds | 2 | Cross | — |
| Partner | PreSales Collective monthly AMA — 60 min | 1 | Seller | `GTM_CONTENT_ENGINE.md` Section 6.4 |
| Float | Contingency | 1.5 | — | — |
| **Total** | | **50** | | |
...
| Block | Task | Hours | Side | Tie-back |
|---|---|---|---|---|
| Product | Asset 4 TCO Calculator (`GTM_CONTENT_ENGINE.md` §5.1 Asset 4) live as standalone `/tools/tco` | 5 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 4 |
| Product | Pre-Scoring Report shareable via M2 pattern — public link supported | 2 | Buyer | M2 extension |
| Product | KB Bootstrap Health Monitor — seller-facing score + weekly email digest | 3 | Seller | Master Spec §22.5 / §48.8 |
| Content | Post 9 publishes — *"How to Evaluate CDP in 2026"* | 0.5 | Buyer | Post 9 |
| Content | Newsletter Issue 9 — *"Founder Journal — First 100 Customers"* | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #1 (honest-artifact reprise), #20 (flywheel math post), #17 (SPOF RFP manager) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 6 planning — Procurement Readiness Self-Assessment content outlined | 3 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 6 |
| Sales (buyer) | Close or walk Enterprise conversation from W8; if close — onboarding sprint immediately | 8 | Buyer | `GTM_SALES_PLAYBOOK.md` §8 |
...
| Podcast | Third appearance — *Sourcing Hero* | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.3 |
...
| Metric | Target | Source |
|---|---|---|
| Buyer Free signups | 160 | `GTM_SALES_PLAYBOOK.md` §1.4 Months 1–3: 40/mo cumulative |
| Seller signups | 300 | `GTM_SALES_PLAYBOOK.md` §1.4: 60/mo cumulative |
| Active evaluations | 90 | — |
| Selection Reports | 20 | — |
| Buyer paid conversions | 4–8 | `GTM_SALES_PLAYBOOK.md` §1.4 |
| Seller paid conversions | 4–8 | `GTM_SALES_PLAYBOOK.md` §1.4 |
| Newsletter subscribers | 1,400 | `GTM_CONTENT_ENGINE.md` Section 4.5 |
| LinkedIn followers | 2,500 | `GTM_CONTENT_ENGINE.md` Section 3.5 Month-3 target |
| M9 category pages live | 6 | M9 |
| M10 guides live | 2 (SIEM, CDP) | M10 |
| Blog posts live | 9 | `GTM_CONTENT_ENGINE.md` Section 7.3 |
| Gated assets live | 5 (Assets 1, 2 full, 3, 4, 5) | `GTM_CONTENT_ENGINE.md` Section 5.1 |
| MRR | $3,000–$8,000 | `GTM_SALES_PLAYBOOK.md` §1.4 |
...
| Product | Starter → Growth auto-upgrade nudge — "3 bids completed; Growth tier unlocks unlimited" (`GTM_PLG_ARCHITECTURE.md` §4.1 nudge #2) | 2 | Seller | `GTM_PLG_ARCHITECTURE.md` §4.1 |
...
| Content | Newsletter Issue 10 — *"Vendor Discovery Beyond G2"* (P4 pillar essay) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #14 (G2 vs. Sourcera), #18 (Apollo vs. Sourcera), #2 (NIST teardown v3) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Asset 6 — Procurement Readiness Self-Assessment shipped (25-question, score-banded report) | 6 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 6 |
| Sales (buyer) | 7 demos; 2 Scale expansions from Month-2 Starter cohort hitting expansion triggers | 12 | Buyer | `GTM_SALES_PLAYBOOK.md` §5 |
| Sales (seller) | 5 demos; Growth-tier conversion push | 5 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
| Sales | 1 Enterprise pipeline touch — ensure 2 active Enterprise conversations by month-end | 2 | Cross | `GTM_SALES_PLAYBOOK.md` §8 |
...
| Product | M13 Heat-Map-driven Loops drip — "what procurement is evaluating this month" monthly newsletter + LinkedIn post pipeline | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 2.5 |
| Content | Newsletter Issue 11 — *"M9 Category Page Walkthrough"* with live heat-map read as lead | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #7 (G2/Gartner carousel v2), #6 (0.3-pt scoring variance), #10 (SOC 2 audit thread revival) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | Guest essay #2 ships — SaaStr — *"Unlimited Seats: The Pricing Decision That Made Our Product a Team Sport"* | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.2 |
| Sales (buyer) | 7 demos; 2nd Enterprise conversation structured (SSO + DPA requirement both confirmed in discovery) | 13 | Buyer | `GTM_SALES_PLAYBOOK.md` §8 |
| Sales (seller) | 5 demos; Scale-tier seller conversation (first Scale seller candidate — 10+ active bids, ≥200 KB entries) | 5 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
| Sales | Cold outbound — 3 inboxes steady; add Sequence F (targeted at Legal/GRC persona at Primary ICP accounts) | 2 | Buyer | `GTM_SALES_PLAYBOOK.md` §2 |
| Partner | First Tier-1 guest essay published — Spend Matters edit ran Week 8; drive amplification | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.2 |
| Community | CISO Series monthly panel appearance — 45 min | 1 | Buyer | `GTM_CONTENT_ENGINE.md` Section 6.4 |
| Podcast | Fourth appearance — *CISO Series Defense in Depth* | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 6.3 |
...
| Block | Task | Hours | Side | Tie-back |
|---|---|---|---|---|
| Product | TCO Calculator (Asset 4) — cross-linked from all 6 M9 category pages + Post 6 inline | 2 | Buyer | `GTM_CONTENT_ENGINE.md` §5.1 Asset 4, M9 |
| Product | 40 more M11 comparison pages — expand to Data/Analytics and RevOps categories | 4 | Cross | M11 |
| Product | Seller Signal Feed (Master Spec §27.9) enhancement — surface categorized anonymized buyer activity to Growth-tier sellers | 4 | Seller | Master Spec §27.9 |
| Content | Post 12 publishes — *"The Ghost-Bid Importer: 12-Month-Old RFP → 47 KB Entries"* | 0.5 | Seller | Post 12 |
| Content | Newsletter Issue 12 — *"TCO Modeling for SaaS"* | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #13 (Ghost-Bid teardown v2), #5 (Selection Memo annotated), #19 (47→183 KB chart) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Content | M12 quarterly report prep — begin aggregation query design for *"State of Enterprise Software Evaluation — Q2 2026"* (k=20 floor) | 4 | Cross | M12, `GTM_CONTENT_ENGINE.md` Section 2.5 |
| Sales (buyer) | 7 demos + Enterprise negotiation (assume 1 Enterprise deal in active proposal stage) | 13 | Buyer | `GTM_SALES_PLAYBOOK.md` §5, §8 |
| Sales (seller) | 5 demos; seller Scale-tier push | 5 | Seller | `GTM_SALES_PLAYBOOK.md` §6 |
| Ops | Weekly review; Win/loss; P1 bug triage | 3 | Cross | — |
| Partner | Webinar #1 planning — *"The 13-Phase Pipeline Applied to Security Tool Evaluation"* (co-hosted with Pavilion) — schedule for Month-4 Week 14 | 2 | Buyer | `GTM_CONTENT_ENGINE.md` Section 4.3 |
...
| Content | Newsletter Issue 13 — *"Founder Journal — 90 Days In"* (the narrative capstone) | 3 | Cross | `GTM_CONTENT_ENGINE.md` Section 4.2 |
| Content | LinkedIn posts — #9 (journal), #20 (flywheel math reprise), #3 (scoring carousel v2) | 2 | Cross | `GTM_CONTENT_ENGINE.md` Section 3.2 |
| Sales (buyer) | 5 demos — reduced intentionally | 10 | Buyer | — |
| Sales (seller) | 4 demos | 4 | Seller | — |
| Sales | Close or walk active Enterprise deals | 3 | Cross | `GTM_SALES_PLAYBOOK.md` §8 |
...
| Partner | Book podcast slate for Month 4 — 5 confirmed appearances | 1 | Cross | `GTM_CONTENT_ENGINE.md` Section 6.3 |
...
| Metric | Target | Source |
|---|---|---|
| Buyer Free signups | 280 | `GTM_SALES_PLAYBOOK.md` §1.4 |
| Seller signups | 540 | `GTM_SALES_PLAYBOOK.md` §1.4 |
| Active evaluations | 180 | — |
| Selection Reports | 45 | — |
| Buyer paid conversions | 10–14 | `GTM_SALES_PLAYBOOK.md` §1.4 |
| Seller paid conversions | 11–18 | `GTM_SALES_PLAYBOOK.md` §1.4 |
| Newsletter subscribers | 2,400 | `GTM_CONTENT_ENGINE.md` Section 4.5 |
...
| Blog posts live | 13 of 20 (Posts 1–13) | `GTM_CONTENT_ENGINE.md` Section 7.3 |
| Gated assets live | 6 (Assets 1, 2, 3, 4, 5, 6) | `GTM_CONTENT_ENGINE.md` Section 5.1 |
| MRR | $12,000–$18,000 | `GTM_SALES_PLAYBOOK.md` §1.4 |
...
### Steady-State Weekly Allocation (Mirrors `GTM_SALES_PLAYBOOK.md` §1.2 + `GTM_CONTENT_ENGINE.md` §1.4)
...
| **Marketing + content** | 10 | 20% | Tuesday content block (2 hrs — long-form or newsletter), LinkedIn × 3 posts (0.75 × 3 = 2.25 hrs), engagement (1 hr Wed), long-form continuation (2 hrs Thu), repurposing (1.5 hrs Fri), community (1 hr) — aligned with `GTM_CONTENT_ENGINE.md` §1.4 |
...
1. **Max 2 demos/day** (`GTM_SALES_PLAYBOOK.md` §1.2). Three demos/day = no deep work that day; five consecutive days at three demos = one week of content loss.
...
The dashboard has three layers: **5 North Star counters** (on the founder's daily home screen), **14 leading indicators** (weekly Wednesday review), and **the full PostHog taxonomy** (monthly deep-dive). This structure matches `GTM_SALES_PLAYBOOK.md` §1.4 and is rendered in Notion + PostHog.
...
The full taxonomy is in Master Spec Appendix G. The events below are load-bearing for PQL scoring and conversion-moment routing:
...
| Scenario | Trigger | Action |
|---|---|---|
| **Strong Green** | MRR ≥$15K AND 1 Enterprise closed AND ≥10 buyer paid AND ≥15 seller paid AND Hero Moment p50 ≤18 min AND M9 pages driving ≥5% of buyer signups | Transition to **Month 4–6 Phase** (separate plan): focus on repeatability. Build first 3 case studies. Begin analyst briefings (`GTM_CONTENT_ENGINE.md` Section 6.1 ladder step 7). Decide on first hire profile (growth engineer vs. GTM associate). |
...
| **Yellow** | MRR in $6–$12K band | **Pricing audit**: re-run pricing narrative through 10 customer conversations; possibly unlock Month-2 drift on price-before-pain discipline (`GTM_SALES_PLAYBOOK.md` §1.2). |
...
3. One of three fixes depending on answer. If (a): re-price Free to 3 concurrent bids ceiling by Month 3. If (b): rewrite day-7/day-30 seller Loops sequence (`GTM_CONTENT_ENGINE.md` §4.4). If (c): rethink Growth-tier value proposition and/or drop the tier.
...
4. Consider: raising the Free-tier AI budget ceiling gating. If Free sellers are burning $5 budgets quickly, reduce to $3 (`Sourcera_Seller_Pricing_Strategy.md` §2.2 permits this strategically).
...
2. Content thesis audit: is the Lead too similar across issues? The 4-issue pillar rotation (P1→P2/P3→P4→P5, `GTM_CONTENT_ENGINE.md` §4.2) is the enforcement mechanism.
...
**Response:**
1. Re-read the Enterprise gate definitions (Master Spec §34; `Sourcera_Buyer_Pricing_Strategy.md` §7): SSO, DPA, ≥$12K AI commit, custom integration. Are any being demanded inappropriately at Scale?
...
**Plan rewrite:** Pricing adjustment is load-bearing — not a spontaneous decision. Book a 2-hour pricing-strategy revision block; consult `Sourcera_Buyer_Pricing_Strategy.md` before any change.
...
**Plan rewrite:** 1 hour/wk from Buyer sales → Marketing. Revise `GTM_SALES_PLAYBOOK.md` §1.2 allocation accordingly.
...
2. On return: Sunday 4-hour block doing three things: (a) re-read `GTM_POSITIONING.md` §5 to reset narrative, (b) re-read `GTM_SALES_PLAYBOOK.md` §1.6 (founder discipline rules), (c) ship one hour of the Method cornerstone rewrite — the single highest-leverage founder activity.
