...
**Source authority:** `Sourcera_Master_Spec.md` §1, §2, §27, §34, §48.0.1, §48, §49 · `Sourcera_Buyer_Pricing_Strategy.md` v3 · `Sourcera_Seller_Pricing_Strategy.md` v3 · `GTM_POSITIONING.md` v1.0 · `GTM_PLG_ARCHITECTURE.md` v1.0.
**Status:** GTM narrative companion for the solo-operator sales motion. Master Spec wins for product, pricing, Marketplace, and commercial-wedge behavior.
...
1. **PLG is the funnel, not sales.** Free tier on both sides is a real product (`Sourcera_Buyer_Pricing_Strategy.md` §4; `Sourcera_Seller_Pricing_Strategy.md` §4; Master Spec §34.1). The Buyer Aha Moment (first Pre-Scoring across ≥3 vendors) and the Seller Hero Moment (bootstrapped bid inside SSO redirect latency) do the first five hours of convincing before a sales conversation starts. Sales time compounds on PQL scoring output (`GTM_PLG_ARCHITECTURE.md` §7), not cold volume.
...
3. **Outcome-accounted AI pricing is self-protecting.** Customers onboard, consume, and churn against a pricing model that cannot produce bill shock (`Sourcera_Buyer_Pricing_Strategy.md` §2.4; `Sourcera_Seller_Pricing_Strategy.md` §2.2; Master Spec §34.3). That collapses the CS load that normally forces a founder to stop selling.
...
4. Of 3–7 invited vendors, 1–3 enter the seller PQL pipeline within 30 days based on cohort data targets (`GTM_PLG_ARCHITECTURE.md` §7.2).
...
1. Seller publishes profile + SellerSoftware entity on signup. Schema.org structured data (Master Spec §26.8 / §26.9.1) emits to public Marketplace + category pages.
...
Target mix at Year-1 exit (combined blended margin ≥ 90%): Buyer 60/25/10/5 across Starter/Growth/Scale/Enterprise; Seller 55/25/15/5 across Starter/Growth/Scale/Enterprise (`Sourcera_Buyer_Pricing_Strategy.md` financial scenarios; `Sourcera_Seller_Pricing_Strategy.md` financial scenarios; Master Spec §34.18).
...
| Payments / billing | **Stripe** (customer-facing) + **Metronome** (consumption accounting, if needed) | 2.9% + 30c | Stripe handles subscription + in-app upgrade flows (`GTM_PLG_ARCHITECTURE.md` §8.1). |
...
- **No demo without a research pre-brief.** The pre-demo research checklist (this playbook §5.1 / §6.1) is run before every booked demo without exception. Saves 25% of demo time.
- **No price before pain.** Pricing is presented after pain is quantified (§8.1). Not before.
- **No Enterprise pitch without the gate.** Do not route to Enterprise until one of {SSO, DPA, ≥$12K AI commit, custom integration} is confirmed as a requirement (Master Spec §34; `Sourcera_Buyer_Pricing_Strategy.md` §7; `Sourcera_Seller_Pricing_Strategy.md` §9).
...
Outbound buyer prospecting targets the five personas from `GTM_POSITIONING.md` §2: Priya (VP Procurement, economic buyer), Marcus (VP IT), Elena (CISO), Dan (CFO), Jordan (Procurement Specialist, champion). Prospecting is concentrated on accounts inside the Primary Buyer ICP (`GTM_POSITIONING.md` §1.1): 500–5,000 employees, $2M–$20M annual software spend, regulated industries.
...
Buyer PQL scoring (`GTM_PLG_ARCHITECTURE.md` §7.1) runs hourly. The founder triages PQLs every morning at 08:00. PQL score thresholds:
...
**Outcome expectations:** ~40% conversion rate to paid on seller PQLs at Conversion Moment #1 with this outreach pattern (Master Spec §48.1.7 / §48.8.6; pricing authority in Master Spec §34). Do not skip this play — it is the most valuable 4 hours of the founder's week.

### 3.3 Direct Outreach to Responsive / Loopio Customers

**Target:** Vendors currently on Responsive (formerly RFPIO) or Loopio with renewals in the next 90 days. The positioning is 4× cheaper with Marketplace discovery bundled (`GTM_POSITIONING.md` §4.2 / §8.7). This is the only buyer-side-style cold outreach that runs on the seller motion.
...
>
> Beta Header and Version Pinning (Master Spec §22.10.7 / §47.3) pins specific agent + skill versions on every call. Your ongoing RFPs see the same model behavior until you intentionally upgrade.
>
...
- **N — Need (trigger event verified).** One of the 8 buyer trigger events (`GTM_POSITIONING.md` §1.1) is actively present. Without an active trigger, the account is in "investigative" mode and closes on a 6–18 month cycle.
...
**Proof point:** Append-Only Grade History (Master Spec §13.5 / §13.10.3), SHA-256 hashed Selection Record (Master Spec §4.3.24 / §10.12), Organizational Intelligence (Master Spec §16) for cross-evaluation discrepancy detection.
...
**Proof point:** 13-phase pipeline (Master Spec §10), Agent Pre-Scoring (Master Spec §13 / §21), Policy-Powered Requirement Generation (Master Spec §12), Enterprise downstream export / integration mapping (Master Spec §31.3 / §32.10.3.F).
...
**Proof point:** Time-Saved Baseline (Master Spec §51.6), Free-tier feature parity for 1 evaluation (`Sourcera_Buyer_Pricing_Strategy.md` §4; Master Spec §34.1.1).
...
**Proof point:** Fixed 13-phase pipeline (Master Spec §10), RBAC model (Master Spec §5), Template Library customization (Master Spec §19).
...
**Proof point:** Dual-Console firewall (Master Spec §1.3 / §7.2), WorkOS SSO (Master Spec §6.1.1), security controls (Master Spec §33), Free tier zero-spend path (`Sourcera_Buyer_Pricing_Strategy.md` §4; Master Spec §34.1.1).
...
**Proof point:** Free tier = 1 active evaluation, full feature set except Opus-gated capabilities (`Sourcera_Buyer_Pricing_Strategy.md` §4; Master Spec §34.1.1).
...
**Proof point:** Data Export (Master Spec §40), Downgrade-safe state (Master Spec §34.5 / §34.6), Pricing margin targets (Master Spec §34.18; `Sourcera_Buyer_Pricing_Strategy.md` financial scenarios).
...
**Proof point:** Reference customer roster (real and anonymized), sample Selection Report, category-creation positioning (`GTM_POSITIONING.md` §6).
...
**Proof point:** Marketplace (Master Spec §27), Marketplace Listings / EOI optionality for buyers (Master Spec §27.2 / §27.5).
...
**Proof point:** Data Residency (Master Spec §1.6 / §40.4 / §47.4), Enterprise custom deployment gate (`Sourcera_Buyer_Pricing_Strategy.md` §7; Master Spec §34).
...
**Proof point:** Pricing anchor (`GTM_POSITIONING.md` §8.7; `Sourcera_Seller_Pricing_Strategy.md` §5-§7), Ghost-Bid Importer (Master Spec §4.4.17 / §22.14.4 / §48.7.4), CRM Sync (Master Spec §31.9), Marketplace (Master Spec §27).
...
**Proof point:** Cite-before-state rule (Master Spec §22.12 / §22.16.1), Confidence thresholds (Master Spec §22.12.3 / §22.16.1), Version pinning (Master Spec §22.10.7 / §47.3), Outcome accounting (`Sourcera_Seller_Pricing_Strategy.md` §2.2; Master Spec §34.3 / §34.15).
...
**Proof point:** KB export (Master Spec §22.18 / §32.9; `Sourcera_Seller_Pricing_Strategy.md` §17.1), KB namespacing (Master Spec §22.8.4 / §22.8.5), Data residency (Master Spec §1.6 / §40.4 / §47.4).
...
**Proof point:** Free tier (`Sourcera_Seller_Pricing_Strategy.md` §4; Master Spec §34.1.2), Ghost-Bid Importer (Master Spec §4.4.17 / §22.14.4 / §48.7.4), Published Seller Profile on Free (`Sourcera_Seller_Pricing_Strategy.md` §2.7 / §4; Master Spec §26 / §34.1.2).
...
> *"Honest answer. The Marketplace is in Year 1 of commercial deployment. Current inventory is concentrated in [list_current_categories_seeded]. We're seeding 6 categories in Year 1 targeting 700+ sellers (`GTM_POSITIONING.md` §1.3).*
>
...
**Proof point:** Category seeding strategy (`GTM_POSITIONING.md` §1.3), Verified/Certified tiers (`Sourcera_Seller_Pricing_Strategy.md` §13.2; Master Spec §26.2 / §34.16.2), M9/M11/M12 organic-traffic loops (Master Spec §48.6).
...
**Proof point:** Tier breakdown (Master Spec §34), conversion moments (Master Spec §48.1.7 / §48.8.6).
...
**Proof point:** KB Staleness Classifier (Master Spec §22.5), KB version chain (Master Spec §22.4), Confidence thresholds (Master Spec §22.12.3 / §22.16.1), Firecrawl re-crawl (Master Spec §22.6).
...
- **Enterprise is always custom.** Volume discounts on committed AI spend follow Master Spec §34.2 and the companion Enterprise sections (`Sourcera_Buyer_Pricing_Strategy.md` §7; `Sourcera_Seller_Pricing_Strategy.md` §9).
...
For each, send the moment-specific email (Master Spec §48.1.7 / §48.8.6; lifecycle email authority in Master Spec §41).
