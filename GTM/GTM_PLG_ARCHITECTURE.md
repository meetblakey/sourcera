...
**Date:** 2026-04-20
**Source-of-Truth:** `Sourcera_Master_Spec.md` §19, §21.4, §22, §26, §27, §34, §48.0.1, §48, §49, §51 · `Sourcera_Seller_Pricing_Strategy.md` v3 · `Sourcera_Buyer_Pricing_Strategy.md` v3 · `GTM_POSITIONING.md` §1–§3
...
**Anti-drift:** This is a narrative companion, not product authority. No new growth mechanics are invented. Every pricing number cites Master Spec §34. The commercial wedge resolves to Master Spec §48.0.1; funnel definitions and thresholds resolve to §51.0.1 / §51.0.3. Every capability referenced is cataloged in `Sourcera_Master_Spec.md §21.4` / §22. Where a surface is silent on an operational question, the gap is flagged as *Authored Extension* and tagged for human sign-off.
...
| B | **Seller arrives via buyer invite** (Forced Signup — M1 / M5 / M15 / M17) | Seller | Peak pain, involuntary, deadline-carrying | p50 < 20 min from magic-link to first submitted response (Master Spec §48.1.6) | Low — bootstrap runs inside SSO redirect latency; landing screen pre-populated | 2nd concurrent bid · 1st EOI · KB 50-entry ceiling (conversion moments, §48.1.7 / §48.8.6) | **18–28%** at 90 days — the single highest-yield channel in the product |
...
**Time-to-value.** TTV is measured from signup to first Pre-Scoring run across ≥3 vendors. Target: **p50 < 72 hours, p90 < 10 days.** The constraint on TTV is not AI availability — the Pre-Scoring capability is Sonnet-tier and returns in seconds — it is the cold-start cost of composing the evaluation matrix. The design lever is Template Library (§19) and Template Discovery (§19.4 / §19.5).
...
**Investment priority.** This is the marquee self-serve path and the primary path for the Wedge Buyer ICP. It must be perfect at launch: templates seeded, Template Discovery visible in Workspace creation, the 7-step Buyer Onboarding flow (Master Spec §35.1) working in ≤90 seconds.
...
**Time-to-value.** Measured in the seller activation metric defined in Master Spec §48.1.6: *minutes from magic-link click to first submitted requirement response.* Target: **p50 < 20 min, p90 < 60 min.** This is the single most-important PLG metric in the product; the Hero Moment (§48.0.1 / §48.8) exists to collapse the 2-day status-quo to 30 seconds of latency hidden behind a progress bar.

**Activation complexity — low, structurally.** The onboarding surface is architected so the seller cannot complete signup without experiencing the Hero Moment (`Master Spec §48.8/§4.4.22). Specifically:
...
**Natural expansion triggers — the conversion moments.** Each is tuned to a different buying reason (§48.1.7 / §48.8.6; pricing authority in §34):
...
**Expected Free→Paid conversion.** 18–28% at 90 days. The range reflects two variables: the M1-effectiveness rate (% of Free sellers who receive a second buyer invitation within 30 days — a leading indicator in `Sourcera_Seller_Pricing_Strategy.md`) and the hero-moment acceptance rate (% of Free sellers whose KB Bootstrap generates ≥40 approved entries — a second leading indicator in `Sourcera_Seller_Pricing_Strategy.md`). A mature Sourcera instance with Marketplace liquidity should clear the upper bound of the range; a cold-start Marketplace in the first six months will land nearer the lower bound because second invitations will be scarce.
...
**Arrival state.** Investigative — the seller found Sourcera via SEO (M9/M10/M11), a published Seller Profile (§26), a Selection Report social share (M2), a Sourcera badge (M3), a bid-success share (M14), a referral (M16), or organic content. They are evaluating RFP tooling; they may also be evaluating Responsive or Loopio.
...
- **M9 Category Pages** — "Best HRMS for Mid-Market," "Best SIEM for Regulated Industries," etc. k-anonymity floor k=5 (Master Spec §4.4.12 / §48.6.4).
- **M10 "How to Evaluate [X]" Guides** — long-form buying guides, draft-generated via `guide_draft_generation` capability, refreshed via `guide_refresh_analysis` (Master Spec §4.4.13).
- **M11 Software Comparison Pages** — AI-generated head-to-head comparisons with Vendor Opt-Out honored (Master Spec §4.4.14 / §27.10).
- **M12 Aggregate Market Intelligence Reports** — quarterly k-anonymized reports (k=20 floor) (Master Spec §4.4.15).
- **M13 Public Marketplace Heat Map** — anonymized category-level demand visualization (Master Spec §4.4.16).
...
**Investment priority.** Medium at launch, high by month 3. SEO compounds. First-mover advantage per category is disproportionate because Google rewards depth and consistency. M9+M10+M11+M12+M13 collectively build the organic-search moat per category that `Master Spec §48` names as the single largest structural defense against Gartner/G2. The category-seeding strategy in `GTM_POSITIONING.md §1.3` sequences the content build — Security/GRC first, then Data & Analytics, then DevOps, etc.
...
Buyer-side activation is designed to deliver the aha moment — the first Pre-Scoring run across ≥3 vendors — in under 72 hours. The $5 AI budget is sized precisely to cover this (Master Spec §34; `Sourcera_Buyer_Pricing_Strategy.md`; `GTM_POSITIONING.md` §8.4). The surface that delivers it is the Master Spec §35.1 Buyer Onboarding Flow.

### 2.1 The Signup Flow — 60 Seconds to First Evaluation

**Step 1 — Landing / Signup (0–10s).** Google SSO or Microsoft SSO via WorkOS (Master Spec §6). No password, no email verification delay (SSO-provided), no credit card, no company-size field, no role-picker modal. Email domain is captured from the SSO identity.

**Step 2 — Org Provisioning (10–15s).** Domain auto-suggest creates the Organization. If the domain matches an existing claimed-domain Org (Master Spec §6.4 Domain Claiming & Verification), the new user is routed into that Org as a pending-approval member via M6 (Domain-Based Auto-Join). Otherwise, a new Org is created with the user as Org Owner. Data residency defaults to US; EU inferred from SSO locale hints (manual override available on Enterprise later).

**Step 3 — Welcome Screen (15–25s).** A single full-screen surface: *"Run your first evaluation."* Three primary CTAs:

1. **"Pick a template"** — opens Template Discovery (§19.4 / §19.5) filtered by the Seed Category list in `GTM_POSITIONING.md §1.3.1`.
...
1. `Cmd+K` — the universal accelerator (Master Spec §3).
...
**Banned in the signup flow** (inherited from `Master Spec §48` seller-side anti-patterns, adapted for buyer-side):
...
1. **Action 1 — Template picked or requirements authored.** The Workspace has ≥20 Requirements. The Template Library (§19) accelerates this — the Sourcera-curated templates seeded at Org creation (Master Spec §19 / §4.4.28) include full requirement sets for each Seed Category. Without a template, the user must write 20 Requirements manually, which is the single largest friction point on the buyer path.
2. **Action 2 — 3+ vendors imported into the Workspace.** Three import surfaces: manual add, CSV import, or Marketplace add (EOI-accepted vendor). The `vendor_invite_suggestion` capability (§21.4.1 / §21.4.5) suggests vendors based on the requirement set and the Workspace's category tags, consuming a small fraction of the $5 AI budget.
...
**Trigger action — first Pre-Scoring run.** The `pre_scoring` capability (§21.4 / §34.11.1) is the single largest buyer-side AI value proof. It reads the requirement + vendor response + supporting evidence and drafts a suggested grade (FM/PM/DNM/EX on the Master Spec §2 rubric) with a confidence score and a cited sentence from the vendor response.
...
- A **Disagreement Insight Card** (`disagreement_insight_card`, §21.4.5) surfaces: *"3 scorers on your team have historically graded 'Data Encryption at Rest' at a wider spread (0.5 variance). Sourcera flagged this Requirement for calibration discussion."*
- A **Time-Saved Baseline** counter (Master Spec §4.3.19 / §50.13) displays: *"This Pre-Scoring run saved approximately 4.3 hours vs. manual scoring. Cumulative time saved this evaluation: 4.3h."*
...
**Target:** p50 < 3 days, p90 < 10 days. This is the direct buyer-side analog of the seller activation metric (§48.1.6). It is the metric that governs the Buyer Onboarding surface, the Template Library investment, and the Evidence Parsing UX.
...
The Template Library (§19) and Buyer Template Discovery (§19.4 / §19.5) exist for one reason: to eliminate the 20-Requirement blank-page cost. `GTM_POSITIONING.md §1.3.1` defines the Seed Categories; each needs a Sourcera-curated template on day one. The operational requirements:

**Pre-launch template seeding (Master Spec §19):**
...
**Template Discovery placement (Master Spec §19.4 / §19.5):**
...
The Hero Moment is spec'd end-to-end in `Master Spec §48.0.1`, `§48.1.5`, `§48.1.6`, `§48.8`, and `§4.4.22` (Seller Onboarding Experience — seven stages). This section translates spec into operational execution — what engineering ships, what Ops configures, what Marketing writes, and how we ensure p50 < 20 min from magic-link to first submitted response.
...
**Trigger.** Buyer invites a vendor via M1 (Vendor-Invite-Creates-Account from Phase 3/4 of their Workspace), M5 (Buyer-Pull Vendor Invite — anonymous templated outreach, Master Spec §48.5.5), or M17 (Buyer-Funded Pro Trial Seat grant, §34.13 / `Sourcera_Seller_Pricing_Strategy.md`). Invite payload carries: `bid_id`, invite target email, buyer Workspace summary, invite signature, and the `seller_domain_hint` derived from the recipient email domain.

**Background steps (t = –N minutes to 0):**

1. **Domain enrichment.** `external_vendor_lookup` (§21.4.1 / §21.4.5) resolves the email domain to public records: company name, industry, HQ region, approximate headcount. Caches to `seller_org_prefill_cache` keyed by domain. Idempotent — reruns do not charge.
...
4. **Invite email delivery** via Loops.so (§41.1 / §41.2). Email subject line: *"You've been invited to respond to [Buyer Company]'s [Category] evaluation."* Email body emphasizes what will happen on click — *"Your response will be drafted from your public documentation in about 2 minutes."* — signaling the Hero Moment before the seller clicks.
...
The seven-stage flow in `Master Spec §48.8/§4.4.22` structures the first session. Operational detail per stage:
...
Progress storytelling is the interstitial surface during Stages 2–3 while the Bootstrap and First-Pass capabilities complete. It must narrate actual work — every status line is true (`Master Spec §48`, "no fabricated steps").
...
**1. Inline citations on every draft.** Every drafted response row shows its KB citation inline — *"from: yourcompany.com/trust/soc2"* with hover expansion showing the exact quoted sentence. This operationalizes the Master Spec §22/§24 "cite-before-state" rule. Engineering-level requirement: every drafted response stores a `citation[]` array with `source_url`, `source_quote`, `kb_entry_id`, and `confidence`. UI binds these to the inline citation chip.
...
- **Edit and accept** — positive signal with measured edit ratio; `outcome: accepted` if edit ratio ≤30% (capability-specific threshold, per Master Spec §34.15 and `Sourcera_Seller_Pricing_Strategy.md`).
- **Reject** — negative signal; surfaces the KB-gap detector (below).

**3. Live budget counter.** Persistent at top-right of the Bid Workspace. Shows both value-dollars consumed and acceptance rate: *"AI budget used: $1.47 of $5.00 · 23 drafts accepted, 3 rejected."* Cost is visible but not alarming — the Free $5 budget is sized to cover exactly one bid end-to-end (`Sourcera_Seller_Pricing_Strategy.md` §14.1).
...
**5. Subtle confirmation chip on accept.** Accepting with no edits surfaces *"added to your KB as verified response"* in a dismissable chip. Reinforces the KB-building mental model. The chip is subtle — no modal, no celebratory animation — to preserve the Linear-style design posture (Master Spec §3).
...
On bid submit, the seller sees a full-screen reveal (`Sourcera_Seller_Pricing_Strategy.md` §15.4):
...
**Stake-reveal mechanics (6 surfaces, per §49.1 Stage 5):**

1. **Now-published seller profile URL** — the domain-verified profile at `sourcera.com/sellers/:slug` (§26 — published profile contract). Copy-link button present.
2. **Live KB count and health score** — 47 entries, 94% health per the KB Health Model (§22.5).
3. **KB Value Meter computed for the first time** — *"Your KB: 47 entries · 6 cited in this bid · Estimated value as reusable response library: $1,275/yr saved response time"* (§22.18.4 / §34.19.2).
...
This is **not** an upgrade CTA. The conversion moments (§48.1.7 / §48.8.6; pricing authority in §34) come later when the natural ceilings hit. The stake-reveal is pure reinforcement — it says, *"look what you built, and look what you can take with you."*

**Success criterion per §49.1 Stage 5:** ≥70% of activated sellers dwell on this screen for >20 seconds. PostHog event: `stake_reveal_dwell_time` with seller_org_id, bid_id, dwell_ms.

### 3.6 The Onboarding Anti-Patterns (Banned, Reinforced)

The seller onboarding surface **must not** do any of the following — any deviation requires formal product review (mirrors Master Spec §48 and §49.1):
...
8. Require profile-publish confirmation — publication is the default on domain verification (§26).
...
These bans exist because supply-side trust is the flywheel. Any friction that reads as sales-ey kills it. `Master Spec §48`: *"the supply-side flywheel depends on vendors trusting Sourcera enough to let an AI touch their buyer's evaluation."*

### 3.7 Operational Levers to Ensure p50 < 20 min

The seller activation metric is *minutes from magic-link click to first submitted requirement response* (Master Spec §48.1.2 / §48.1.6). Target: **p50 < 20 min, p90 < 60 min.** Each of the following operational levers has a direct, measurable effect on this metric.
...
The Seller Activation dashboard reports daily: funnel by stage, p50/p90/p99 elapsed time per stage, Stage 3 population success rate (P0 availability metric, alerts on-call if 90th-percentile Stage-3 population latency exceeds 10s per §49.1).

---

## SECTION 4 — Conversion Trigger Architecture

Conversion triggers are the moments Sourcera asks a Free user to pay. Every trigger in this section is spec'd in Master Spec §34 or `Sourcera_Seller_Pricing_Strategy.md` §14.3. This section designs the *operational execution* — when to fire, what copy to show, where to place the CTA, and how to avoid the dark patterns the anti-pattern ban prohibits.

### 4.1 Buyer Free → Starter Triggers

Four structural triggers fire the Free → Starter upgrade conversation on the buyer side (`Master Spec §34`). Each has a contextual, non-generic CTA placement and copy.
...
**Trigger 3 — AI budget exhaustion ($5).** Fires when the 50%, 80%, and 100% thresholds on the Free $5 budget are crossed (Master Spec §34.8.4 / §34.10.3).
...
**Trigger 4 — Opus-tier capability attempted.** Fires when the user attempts Policy Parsing, Deep Comparison, or any other Opus-gated capability cataloged in §21.4. These consume the $5 budget in one call if allowed and are pre-emptively gated on Free.
...
The conversion moments are the central mechanic of the seller funnel (Master Spec §48.1.7 / §48.8.6; pricing authority in §34). Each is tuned to a different buying reason. The product must catch all three — the pricing model does not optimize any single one.
...
- **Trigger:** Seller clicks *"Submit EOI"* on any Marketplace listing (§27.5).
...
- **Expected conversion timeline:** 40–55% inside 21 days. This is investment-coded and the single highest-converting of the three moments, because the KB Value Meter (§22.18.4 / §34.19.2) has been accruing value the whole time.

### 4.3 The Win/Loss Debrief as Upgrade Trigger (the fourth moment, non-threshold)

The win/loss debrief (`Sourcera_Seller_Pricing_Strategy.md` §15.6; Master Spec §48.7) fires on bid close (won or lost, as reported by the buyer or defaulted to `status_unknown` at Phase 13 entry). It is the only non-threshold conversion surface in the seller funnel — it fires regardless of whether the seller has hit a ceiling.
...
**Placement:** Email (Loops.so, §41.1 / §41.2) within 60 minutes of bid close; in-app Pulse event; full-screen debrief surface on next login. The in-app debrief surface is the highest-converting; the email is the reminder.
...
Expansion within the paid tier ladder (`Master Spec §34`). Each trigger is usage-based — the product surfaces the upgrade when the usage data makes the business case obvious.

**Buyer Starter → Growth:**

- **Primary trigger:** 80% AI budget utilization ($40/$50) for 2 consecutive months. This is the canonical expansion signal (Master Spec §34.1 / §34.10 and `Sourcera_Buyer_Pricing_Strategy.md`).
...
- **CTA placement:** Monthly Pulse Digest email to Org Owner + Billing Admin; in-app banner on the Usage Dashboard (Master Spec §34 required feature #6).
...
Enterprise is gated (Master Spec §34 and `Sourcera_Buyer_Pricing_Strategy.md`). The trigger is **not** size — it is any one of {SSO required, DPA required, ≥$12K/yr AI commit, custom integration}. The operational challenge is identifying these triggers before the user churns from Scale.
...
- DocuSign contract drafted from the Enterprise MSA template (Master Spec §34 Enterprise contract path, custom DPA appended if required).
- Closed contracts hand off to the dedicated CSM for onboarding (or founder-run CSM in the first 10 Enterprise accounts).

**Usage data → founder outreach trigger (the automation).** A weekly script runs against the Usage Dashboard data (§51.3 / §51.4 / §51.5) and identifies any Scale customer meeting at least one of the four triggers. Output is a Slack alert to the founder's DMs with a direct link to the account and the trigger. This is the single most important automation in the Enterprise-routing playbook — without it, trigger detection relies on ad-hoc salesperson intuition, which does not scale below a team of 5.

---

## SECTION 5 — The 17 Growth Mechanics — Operational Execution Plan

Each mechanic M1–M17 is spec'd in `Master Spec §48`. This section adds the operational wrapper: current-implementation assumption (what must be true in the product), the GTM action the solo operator takes to amplify it, the metric that measures its effectiveness, and the launch priority.

### M1 — Stakeholder Read-Only Invite

- **Implementation assumption:** Magic-link invite surface in Phase 8+ artifacts; read-only auth path via WorkOS; artifact types include Selection Report (§4.3.23 / §10.12), Scoring Matrix (§13), TCO Summary (§15), and stakeholder summary capabilities in §21.4. Recipients land in a restricted read-only Workspace view with no edit capability.
...
- **Implementation assumption:** Anonymous templated outreach surface — the buyer enters a vendor's email or domain, Sourcera sends a templated magic-link invite on the buyer's behalf. Does not expose buyer identity until the vendor accepts. Spam controls: per-buyer rate limit (10/week on Free, higher on paid), vendor opt-out registry check (Master Spec §27.10 / §4.4.8), domain validation against shared-use domain list.
...
- **Implementation assumption:** When a new user signs up from a claimed-domain (verified via DNS TXT, Master Spec §6.4), they are routed to the existing Org as a pending-approval Member. Org Owner receives an approval request.
...
- **Implementation assumption:** The `team_suggestion` capability (§21.4.1 / §21.4.5) surfaces "your colleagues are also on Sourcera" prompts in the onboarding flow and in the Workspace create modal when it identifies other Org members from the same company domain or shared Slack workspace.
...
- **Implementation assumption:** Organizational Intelligence (§16) surfaces compounding analytical value per additional evaluation — cross-evaluation discrepancy detection, requirement frequency analysis, vendor scoring deltas. Visible in-app via a dedicated Org Intelligence dashboard.
...
- **Implementation assumption:** AI-generated, editorially-reviewed Category Pages via `category_faq_generation` capability (§21.4 / §4.4.12). Each page surfaces: FAQ (Agent-drafted, editor-reviewed), vendor list (published Seller Profiles in that category), "How to Evaluate [X]" link to M10 guide, 3 Comparison Page links (M11), Heat Map snippet (M13). Schema.org `Product`, `FAQPage`, and `ItemList` structured data emitted.
...
- **Implementation assumption:** Sellers paste historical RFPs (Word, PDF, CSV); `ghost_rfp_ingestion` (§21.4.2 / §22.10.6 / §22.14.4) extracts requirements and drafted responses; outputs feed the seller's KB. First import free per Seller Org (M15 growth mechanic + `Sourcera_Seller_Pricing_Strategy.md`).
...
The 10 loops are defined in `Master Spec §48`. Two slots (3 and 6) are retired and replaced by M2 and M7 respectively. This section provides the measurement framework: loop time, branching factor, leading indicator, accelerants, killers.

### Loop 1 — Vendor-Invite-Creates-Account

- **Mechanic:** Every buyer-invited vendor receives a magic-link signup; first bid seeds KB via KB bootstrap (§22.10.3 / §22.14).
...
- **Killers:** Low buyer-side template-discovery visibility (Template Discovery must be prominent in Workspace creation — if buried, Loop 2 dies); template spam (ML classifier per `Master Spec §48` — if false positives spike, good templates get suppressed).
...
- **Accelerators:** KB health score on the seller side (better KB → better auto-enriched page content → higher ranking); Capability Declaration density; Verified/Certified tier upgrades (improve page trust signals); seller page enrichment (§26.7 / §21.4.2).
...
- **Mechanic:** Command Palette (Master Spec §30 / §48.2) surfaces relevant Marketplace content based on current context — a Requirement viewed surfaces matching Seller Pages; a Use Case surfaces category templates; a Workspace surfaces category vendors.
...
- **Accelerators:** Rich Capability Declaration density (more structured data → better context matching); indexed KB-to-Capability mapping (§22.10.5 / §26.4); Marketplace category density.
...
All self-serve monetization flows route through Stripe. The AI Wallet service (Master Spec §34 required feature #1) holds the included-budget counter, overage counter, and auto-topup config Org-scoped and pooled across consoles (Master Spec §34; `Sourcera_Seller_Pricing_Strategy.md` §11).
...
6. **Confirmation.** Immediate plan activation (<30 seconds). The Org is upgraded in real-time; all carry-over data (KB, evaluations, vendors, in-flight bids for cross-console orgs) is preserved; acceptance record in the Audit Log (§6.7).
...
The AI Wallet is **off by default on every plan** (Master Spec §34; `Sourcera_Buyer_Pricing_Strategy.md` §7). Admin enables explicitly.
...
Downgrade from paid → Free or paid → lower paid tier (Master Spec §34.5 / §34.6; `Sourcera_Buyer_Pricing_Strategy.md` and `Sourcera_Seller_Pricing_Strategy.md` narrative companions).
...
2. **Downgrade confirmation.** A full-screen confirmation shows:
   ...
   - The 90-day preservation guarantee: *"Data in read-only state is preserved for 90 days. During this period, you can upgrade at any time and all read-only data becomes active again automatically. After 90 days, read-only data is archived per our Data Retention policy (§40.2)."*
...
- Any downgrade path that does not preserve audit log entries (§6.7 — audit logs retain full history regardless of plan).
- Hiding the downgrade option behind customer-support-only contact flows.

### 8.6 Pro Trial Seat Mechanics (M17)

Per Master Spec §34.13 and `Sourcera_Seller_Pricing_Strategy.md`.
...
- Magic-link lands the vendor in Stage 1 of Seller Onboarding (§49.1).
...
**Stripe webhook handling:** All Stripe events handled idempotently via `event_id` keyed in a `stripe_webhook_log` table. Retry with exponential backoff; DLQ after 5 failures per Master Spec §31/§32 webhook pattern (the same pattern applied internally to Stripe inbound).

**Per-console plan assignment:** Plans activate per console, not per Org (Master Spec §34; `Sourcera_Seller_Pricing_Strategy.md` §11). A single Stripe Customer holds multiple Subscriptions — one Subscription per active console (Buyer, Seller) — sharing one payment source. AI Wallet balance is Org-scoped and pooled across both consoles' budgets (Master Spec §34).
...
- [ ] 7-step Buyer Onboarding live (Master Spec §35.1)
...
- [ ] 7-stage Seller Onboarding live (§49.1)
...
- [ ] Published-from-Day-One Seller Profile default (§26)
...
- [ ] Magic-link SSO orchestrator with parallel bootstrap (§49.1 required capability)
...
- [ ] DMARC / SPF reputation, shared-use domain detection, spam ML classifier (Master Spec §48.4)
- [ ] Vendor Opt-Out Global Registry (Master Spec §27.10 / §4.4.8)
- [ ] Signal Integrity Monitor (Master Spec §50.15 / §48.4)
...
- [ ] DSAR and right-to-erasure compatibility preserved (§40.2)
- [ ] Honest KB Portability guarantee (§34.19)
...
**Document version.** 1.0 (2026-04-20). Supersedes any prior PLG draft. Changes require reconciliation against `Sourcera_Master_Spec.md` §22 / §27 / §34 / §48 / §49, the buyer and seller pricing v3 companions, and `GTM_POSITIONING.md` per the Source-of-Truth Hierarchy in `GTM_Project_Instructions.md`.
