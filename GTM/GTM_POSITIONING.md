...
**Source authority:** `Sourcera_Master_Spec.md` §1, §2, §27, §34, §48.0.1, §48, §49 · `Sourcera_Buyer_Pricing_Strategy.md` v3 · `Sourcera_Seller_Pricing_Strategy.md` v3.
...
1. **An auditor finding that cited inconsistent vendor evaluation or missing decision documentation.** Fastest-converting trigger. The buyer has 30–90 days to produce a remediation plan. Sourcera's SHA-256 hashed Selection Record (§10.12 / §4.3.23 / §4.3.24) and complete audit log (§6.7 / §4.6.1) are the exact artifacts the auditor is asking for.
2. **Failed or contested vendor evaluation where the selected vendor was sued, protested, or produced a post-facto justification problem.** The buyer has institutional memory of why this went wrong. Sourcera's Append-Only Grade History (§13.5 / §13.10.3), Disagreement Insight Cards (§13.6), and immutable Phase 9 snapshots (§10) map directly.
...
4. **Compliance mandate — SOC 2 Type II, ISO 27001, HIPAA, NYDFS 500, DORA, PCI-DSS 4.0, FedRAMP readiness — that forces documented vendor-risk procedures.** Policy-Powered Requirement Generation (§12) is the unlock. Upload NIST 800-53, ISO 27001, or SOC 2 control set; the Agent extracts structured requirements with traceability back to source control.
5. **An upcoming top-5 software purchase of $1M+ TCO.** The buyer is building a spreadsheet *now* and knows this one will be scrutinized. Sourcera's TCO Modeling (§15) with structured pricing inputs (flat, tiered, one-time, percentage-of-license, estimate-range, discount) and Scenario Modeling (§14) are the conversion hook.
...
7. **Merger, acquisition, or carve-out creating duplicate SaaS contracts.** Organizational Intelligence (§16) surfaces cross-evaluation discrepancy detection — the exact capability needed to rationalize overlap.
...
Sourcera's displacement argument is not "we are better than Coupa." It is "Coupa is a P2P suite; the evaluation is still happening in spreadsheets beside it." The Dual-Console architecture (§1.3), the fixed 13-phase pipeline (§10), and the Agent's 21-capability catalog (§21.4) are the category-defining assets — none of the replaced tools have them.

#### 1.1.4 Internal Team Structure for Software Purchases

The typical Sourcera buying team contains 5–9 distinct roles, spanning 3–6 functions. This is why Sourcera rejects per-seat pricing: a seat tax suppresses exactly the cross-functional participation the product requires. The roles map into Sourcera's RBAC model (§5):
...
- **InfoSec / Security Evaluator** (CISO office, Security Engineer, GRC Analyst) — owns InfoSec Team queue (§8.4 / §20.3); reviews SOC 2, pentest, and DPA inputs; uploads compliance frameworks for Policy-Powered Requirement Generation (§12).
- **Legal Evaluator** (Commercial Counsel, Legal Operations) — owns Legal Team queue; reviews DPA, BAA, MSA terms; owns Custom Agent Instructions for contract-language flags (§21.8).
- **Finance Evaluator** (FP&A Analyst, Finance Business Partner) — owns TCO Use Cases (§15); validates pricing structures and multi-year projections.
- **End-User / Line-of-Business Representatives** (3–8 functional leads) — score capability requirements in their domain.
- **Billing Admin** (separate role from Org Owner; added in RBAC for wallet, AI spend caps, billing events — §5.2.1).
...
- Highly classified or air-gapped environments where Convex cloud hosting is disqualifying. Enterprise custom residency (§40.4) only partially solves this; route only to Enterprise with custom deployment scope.
...
2. **Lost an RFP because of response latency.** "We would have won if we had answered in time." Sourcera's First-Pass RFP Response Generator (§22.14.1 / §24) with grounded KB citations collapses first-draft time from days to minutes.
...
4. **Moving upmarket — first bid to an enterprise buyer with a SOC 2 Type II + ISO 27001 requirement list.** KB Bootstrap (§48.8 / §49.1) plus KB-to-Capability Auto-Suggestion (§26.4 / §22.14.3) supply the exact structured answer fabric needed.
5. **Entering a new category or vertical where the seller has no template library.** Ghost-Bid Importer (M15, §4.4.17 / §22.14.4 / §48.7.4) lets the seller paste historical RFPs and bootstrap a KB in one session — the only tool in this category that does this.
...
7. **New head of sales / new head of revenue operations in seat.** Similar to the buyer-side leadership trigger — new leader wants a modern stack. CRM Sync (§31.9) with Salesforce, HubSpot, Dynamics, Pipedrive is the hook.
8. **Seller wants quantified demand signals.** Seller Signals (§27.9) — k-anonymized buyer intent routed to CRM — is the only tool in this category that sources signals from inside active evaluations rather than inferred intent data.
...
|---|---|---|---|
| 1 | **Security & GRC** (SIEM, SOAR, XDR, GRC platforms, DLP, IAM, PAM, CSPM) | Highest-friction evaluations, heaviest compliance documentation burden, most acute Policy-Powered Requirement Generation (§12) fit | 200+ sellers across 8 sub-categories |
| 2 | **Data & Analytics** (BI platforms, CDPs, reverse ETL, data observability, data catalogs, data governance) | Fast-growing category with weak category pages on G2; structured TCO (§15) is a strong wedge | 150+ sellers |
| 3 | **DevOps & Platform Engineering** (CI/CD, IaC, secrets management, internal developer platforms, observability) | Engineering buyers prefer structured evaluation; sellers have excellent public docs for KB Bootstrap (§48.8 / §49.1) | 150+ sellers |
| 4 | **Revenue Operations** (Sales engagement, CRM adjacent, revenue intelligence, CPQ, partner management) | Dense existing competitive landscape; CRM Sync (§31.9) plus Seller Signals (§27.9) make Sourcera more valuable to the seller than Responsive | 100+ sellers |
...
**Year 1:** US-headquartered sellers selling to US buyers. Single-region data residency (AWS us-east-1 / us-west-2). Coverage of EU-GDPR-compliant sellers by year-end via existing `data_residency_region = EU` support (§40.4) but without a dedicated EU GTM motion.
...
Inside the category seeding above, Sourcera targets **regulated industries first** because the audit-trail, policy-parsing, and Selection Record features (§10.12 / §4.3.23 / §4.3.24) are disproportionately valuable there. Order of vertical priority on the buyer side:
...
Each persona below is an economic buyer, influencer, or champion on the buyer side. Personas map to Sourcera's RBAC roles (§5) where applicable and to specific Sourcera features that solve their day-to-day pain. Every objection is paired with a feature-grounded rebuttal — no generic "ROI-based" answers.
...
- **What Sourcera solves for Priya.**
  - **Fixed 13-phase pipeline (§10)** — forces stakeholder discipline with phase gates; no skipping Phase 1 stakeholder alignment or Phase 9 immutable snapshots.
  - **Pulse Health Score (§20.3)** — a single 0–100 number tracking SLA Compliance, Scoring Progress, Response Rate, and Phase Velocity. Weekly Pulse Digest email to Priya and her exec sponsor (§20.4).
  - **Selection Report + SHA-256 hashed Selection Record (§10.12 / §4.3.23 / §4.3.24)** — the exact artifact the auditor asks for; replaces the 10-hour PowerPoint.
  - **Append-only grade history (§13.5 / §13.10.3)** — every scoring decision, override, and rationale retained with timestamp, user, justification. Audit-ready by construction.
  - **Organizational Intelligence (§16)** — surfaces when the same vendor was scored differently on the same requirement in two evaluations, flagging scoring inconsistency before an auditor does.
  - **Unlimited seats on every paid tier (Buyer Pricing Strategy v3 §2.1 / Master Spec §34.1.1)** — Priya can invite Legal, InfoSec, Finance, and IT without a procurement battle over license count.
- **Objections and rebuttals.**
  - **"We already have Coupa/Ivalua/Jaggaer."** — Coupa is a P2P suite; Ivalua is a sourcing suite. Neither shipped a fixed 13-phase evaluation pipeline, Agent Pre-Scoring (§21), Policy-Powered Requirement Generation (§12), or a Vendor Discovery Marketplace. Your evaluation is still happening in spreadsheets beside Coupa — we replace the spreadsheet, not the P2P backbone. Coupa/Ivalua integration is available on Enterprise (§32) so the Selection Report routes downstream.
  - **"We don't have budget for a new tool."** — Business Starter is $299/mo annual with unlimited seats. That is less than one Coupa seat for one year. You are not adding a stack; you are replacing the spreadsheet + SharePoint + Outlook + PowerPoint workflow that is consuming 120+ hours per evaluation. The Time-Saved Baseline (§51.6) will reconcile the economics in your first evaluation.
  - **"How is this different from a spreadsheet with more features?"** — Spreadsheets cannot enforce a phase gate. They cannot produce a SHA-256 hashed audit artifact. They cannot run Pre-Scoring. They cannot detect a scoring disagreement (≥0.3 variance) and flag it for the Use Case Lead. They cannot export to Jira / Linear / Salesforce at Phase 13 close. Every sentence in that list is a real, shipped Sourcera feature (§13 / §21.4, §13.6, §31.3 / §32.10.3.F).
  ...
  - **"What about AI hallucination in our audit?"** — Every Agent output is labeled `[AI-Generated]` with a confidence score (§21.4 / §21.6). Outputs never commit without explicit user confirmation. Pre-Scoring shows the Agent's suggested grade; the reviewer accepts, overrides, or dismisses. Override rates are tracked per Use Case for model calibration (§21). Rejected outputs bill at cost price, not value price, so Sourcera's economic incentive is aligned on quality (Buyer Pricing Strategy v3 §2.4 / Master Spec §34.3).
...
- **What Sourcera solves for Marcus.**
  - **Domain Governance (§6.4)** — verified corporate domains block unsanctioned Org creation; forces employees onto a single Org and surfaces shadow-IT signups into IT's approval queue.
  - **IT Team horizontal track (§8.4 / §20.3)** — an IT Team owns triage queues, SLA enforcement, and Custom Agent Instructions across all active Workspaces. Marcus writes a rule once — *"flag any vendor response that mentions single-tenant hosting"* — and it applies everywhere (§21.8).
  - **Policy-Powered Requirement Generation (§12)** — upload NIST 800-53 once; Marcus never re-authors the same 40 control questions across 6 evaluations.
  - **Post-Phase-13 export to Jira / Linear / Asana / Azure DevOps (§31.3 / §32.10.3.F)** — the selection outputs actually reach the engineering team instead of dying in the Selection Report.
  - **Template Library (§19)** — seeded templates for CRM, HRMS, ERP, Security Tools, Dev Tools eliminate the blank-page cost of every evaluation.
- **Objections and rebuttals.**
  - **"I don't want another tool I have to onboard my team into."** — Unlimited seats means your team joins at zero marginal cost. The Linear-benchmark design language (Master Spec §3) is Cmd+K-first, J/K keyboard-nav, optimistic mutations — your platform engineers will recognize it. Onboarding is 60 seconds to first evaluation (§35.1).
  - **"Our security team hasn't reviewed this."** — WorkOS SSO (§6.1.1), SOC 2 Type II on the 12-month roadmap, ISO 27001 planned, annual penetration testing, Convex-backed data isolation at the query layer. Dual-Console firewall (§1.3) enforces buyer-seller isolation at the database layer, not just UI. SBOM available to Enterprise on request. Full security posture documented in the SPA package sent with the first paid proposal.
  - **"Can this integrate with our existing stack?"** — REST API v1 (§32), bi-directional CRM sync on the seller side (§31.9), Jira/Linear/Asana/Azure DevOps/Salesforce native integrations (§31.3 / §32.10.3.F), Slack + Teams channel-scoped notifications, webhooks with HMAC-SHA256 signatures and DLQ after 5 failures. The API add-on is $99/mo on any Business tier (Master Spec §34).
...
- **What Sourcera solves for Elena.**
  - **Policy-Powered Requirement Generation (§12)** — Elena uploads SOC 2 / ISO 27001 / NIST 800-53; the Agent extracts structured Requirements with `policy_source` pointers. Once a framework is mapped, updates to the policy flag downstream Requirements automatically. This is the single highest-value capability for CISOs — it takes 40 hours of manual translation off the critical path per evaluation.
  - **InfoSec Team horizontal track with Custom Agent Instructions (§21.8)** — Elena writes *"flag any vendor response mentioning shared-tenant hosting or third-party data processors not in our approved list"* once; the Agent enforces it across every active evaluation in under 60 seconds of save.
  - **Agent Pre-Scoring (§13 / §21.4)** — the InfoSec review team sees suggested grades on every qualitative security requirement before they open the scoring UI. Cuts review time by roughly half on standard controls.
  - **Traceability Matrix in the Selection Report (§10.12 / §4.3.23 / §4.3.24)** — every compliance requirement traces back to the source policy control. Audit-ready by default.
  - **Data Residency per Org (§40.4)** — US, EU, or Custom. EU-backed Orgs stay in AWS eu-west-1 / eu-central-1.
  - **Dual-Console Firewall (§1.3)** — seller-side AI cannot access buyer-side data and vice versa. Enforced at the database query level, not UI.
- **Objections and rebuttals.**
  - **"I can't upload SOC 2 reports to a new SaaS tool."** — Data residency is per-Org; EU-region-locked customers stay in EU. All uploads are virus-scanned and strict-Markdown sanitized (no raw HTML, no external images, no scripts — §33.1 / §33.6). Audit logging is SHA-256 chained. Enterprise customers can require HIPAA BAA and custom DPA. If these controls do not satisfy your review, we will not push the evaluation forward.
  - **"How is AI not introducing a new compliance risk?"** — Every Agent output is labeled, confidence-scored, and editable. Outputs are filtered against factually incorrect pre-scores, unseen vendor names, and direct scoring recommendations (§21). The Agent never commits without explicit user confirmation. The model runs in Anthropic's security-certified infrastructure; SOC 2 Type II targeted within 12 months.
  - **"This is just another tool I have to approve."** — It is one tool to approve that replaces six uncontrolled surfaces (Google Drive, Outlook, SharePoint, PowerPoint, Excel, Word). The net-new attack surface is smaller, not larger. Full OWASP Top 10 mitigations documented (§33).
...
- **What Sourcera solves for Dan.**
  - **TCO Modeling as a first-class Use Case type (§15)** — six structured pricing schemas (flat, tiered, one-time, percentage-of-license, estimate-range, discount). Multi-year projections with configurable time horizons (1/3/5 year) and seat trajectories. Dan sees apples-to-apples cost comparisons before the vendor is selected.
  - **Scenario Modeling (§14)** — Dan can overlay "what if seat growth runs at 20% instead of 10%" and watch rankings shift. 10 scenarios on Business, 50 on Enterprise.
  - **TCO Breakdown Report inside the Selection Report (§10.12 / §4.3.23 / §4.3.24, §15.3.4)** — per-vendor decomposition across license, implementation, support, training, and overage with first-year and steady-state callouts.
  - **Organizational Intelligence (§16)** — historical spend patterns and vendor performance surface when the same vendor is evaluated twice.
  - **Billing Admin role (§5.2.1)** — Dan's office owns the AI Wallet, spend caps, and overage. Wallet is off by default; auto-topup never enabled by default (Master Spec §34).
- **Objections and rebuttals.**
...
- **What Sourcera solves for Jordan.**
  - **SLA timers with tiered escalation (§8.4.2 / §20.3)** — 80% warning to owner, 100% to Team Lead, 125% to Workspace Owner, 150% auto-reassignment. Jordan stops having to chase by email.
  - **Inbox & Pulse (§20)** — unified notification center with per-channel preferences. Slack, Teams, email, in-app.
  - **Agent Pre-Scoring, Q&A Answer Suggestion, Requirement Splitting (§21.4 capabilities 4, 5, 9, 10)** — the Agent does the administrative overhead so Jordan does the coordination.
  - **Auto-generated Selection Report (§10.12 / §4.3.23 / §4.3.24)** — built from scores, TCO, Scenario Modeling, policy traceability. Exports to PDF. Replaces the 10-hour PowerPoint.
  - **Command Palette (§30)** — `Cmd+K` navigation across every entity. Jordan's productivity multiplier.
  - **Real-time collaborative scoring (§13.6.1)** — team members grade simultaneously with live presence. No more "chase people for scores."
...
- **Objections and rebuttals.**
  - **"I don't want to learn a new tool mid-evaluation."** — Start on Free with your next evaluation, not the one you are already running. Templates are seeded at Org creation (§19.2 / §19.5) — CRM, HRMS, ERP, Security Tools, Dev Tools. You configure the first evaluation in under an hour.
  - **"Will my stakeholders actually use it?"** — Stakeholders do not need to learn a tool. Guest role (§5.4) has four permission profiles (`read_only`, `contributor`, `scorer`, `full_participant`) scoped to specific Use Cases. An exec sponsor receives a magic-link invite (M1), approves or signs via a simple UI, never sees the phase sidebar or the KB. Unlimited seats means you pay zero marginal cost per additional stakeholder.
...
- **What Sourcera solves for Sam.**
  - **First-Pass RFP Response Generator (§22.14.1 / §24)** — drafts responses to every buyer requirement using the seller's KB. Cite-before-state rule. Typically covers 40–70% of requirements with high-confidence draft on a bootstrapped KB.
  - **KB Bootstrap (§48.8 / §49.1)** — on Seller signup, Opus-tier crawl of the seller's homepage, help docs, and trust center generates a proposed KB from scratch. First bootstrap is free for every new Seller Org.
  - **CRM Sync (§31.9)** — Salesforce, HubSpot, Dynamics, Pipedrive. Account matching algorithm, routing rules, bi-directional activity sync. Seller Signals (§27.9) — k-anonymized buyer intent — flow directly to the right CRM Account.
  - **Marketplace Listings + EOI workflow (§27)** — Sam's team submits EOIs to buyer-published opportunities. Verified or Certified badge lifts match score weight.
  - **Published Seller Profile from day one (§26.1 / §26.7)** — live on domain verification, Schema.org structured data for SEO, no paywall to publication.
- **Objections (Organic path) and rebuttals.**
  ...
  - **"AI is going to hallucinate in our RFP response and we'll look bad."** — Every draft is grounded in KB entries with inline citation on every response row: *"from: yourcompany.com/trust/soc2"* with hover expansion showing the exact quoted sentence. Ungrounded statements are dropped by the cite-before-state rule (§22.14.1 / §24). The Agent never commits; your team accepts/edits/rejects each response. Confidence scores are visible (§21.4 / §21.6). On rejected drafts, a KB-gap detector lets you write the real answer inline and save it as a KB entry in one step.
  - **"Our KB is our IP. I don't want it locked into your platform."** — KB is exportable in full — Markdown bodies, JSON schema, entry metadata manifest, provenance log, zipped attachments — at every tier, including Free. One click. Never paywalled. Documented publicly (§22.18 / §32.9). What exports can't capture — confidence scores earned from accept/reject signals, staleness state, win-rate weighting, cross-bid citation graph, KB-to-Capability auto-declarations — is on-platform compounding. We win by making the live KB more valuable than the export, not by holding the export hostage.
...
- **What Sourcera solves for Aisha.**
  - **KB Core with Document Library (§22.3 / §22.7)** — centralized repository for SOC 2, ISO certs, DPAs, pentest summaries. Expiration dates, version chains, auto-flagging of bids referencing superseded versions.
  - **Automated Documentation Crawling via Firecrawl (§22.7 / §22.14)** — weekly crawl (Starter), daily crawl (Growth), real-time (Scale). Changes in previously crawled pages flag affected KB entries for re-verification.
  - **Self-Governing Health Model (§22.5)** — every entry carries a `confidence_modifier` that decays with age. Stale entries deprioritized automatically. Entries with low win rates auto-deprioritized.
  - **Cross-bid citation graph (§22.18 / §32.9)** — exposes which entries are load-bearing. Aisha sees which KB entries are cited across which bids and which win.
  - **KB MCP Server with namespaced retrieval (§22.8.4 / §22.8.5 / §22.10)** — hybrid dense + lexical, server-side query expansion, re-rank model, pre-filter by `seller_org_id` before vector search, not after. Cite-before-state rule enforced.
- **Objections (Organic path) and rebuttals.**
  ...
  - **"We've been burned by AI making up security claims."** — Sourcera's cite-before-state rule drops any response sentence without KB grounding. Every draft row shows its citation. Confidence thresholds per capability (KB suggestions 70%, Evidence parsing 60% — §21.4 / §21.6) suppress low-confidence output. Managed Agents are versioned and skill-pinned (§22.10 / §22.10.7 / §47.3); behavior does not drift silently across deployments.
  - **"We need to pin versions of the AI — we can't have behavior change mid-RFP."** — Beta Header & Version Pinning (§22.10.7 / §47.3) pins specific agent + skill versions on every call. Your ongoing RFPs see the same model behavior until you intentionally upgrade.
...
- **What Sourcera solves for Leo.**
  - **Hero Moment on buyer-invite (Forced-Signup path, §48.8 / §49.1)** — Leo arrives at Sourcera via a buyer invite. Inside the SSO redirect latency, the Agent crawls his company's public surfaces, proposes 40–200 draft KB entries, and drafts responses for every buyer requirement. He lands on: *"Your bid for [Buyer Company] is ready. 47 of 82 requirements have AI drafts. 31 cite evidence from your own website."*
  - **Bid Task & Bid Schedule (§4.4.5 / §4.4.6)** — each requirement becomes a Bid Task with owner, due date, status. Bid Schedule rolls into a Gantt-style view with SLA markers.
  - **Seller Pulse (§24.4)** — four-signal health score per active bid (response completion, SLA compliance, KB coverage, Q&A responsiveness).
  - **Ghost-Bid Importer (M15, §4.4.17 / §22.14.4 / §48.7.4)** — Leo pastes or uploads historical RFPs; the Agent runs Ghost-RFP Ingestion (Opus) and seeds his KB from prior work. First import free per Seller Org. Catapults Leo over the KB-accumulation barrier in one session.
  - **Win/Loss Debrief (Seller Pricing Strategy v3 §15.6 / Master Spec §22.18)** — on bid close, Leo sees which KB entries were cited in winning/losing responses. Win reinforces value. Loss identifies specific gaps for upgrade.
  - **KB Value Meter (§22.18 / §32.9, Seller Pricing Strategy v3 §17.3 / Master Spec §34.19.2)** — persistent meter: *"Your KB: 47 entries · 6 cited in closed bids · 2 with win-rate lift · Estimated value: $1,275/yr saved response time."*
...
- **What Sourcera solves for Rachel.**
  - **Firecrawl crawling the website as the canonical source (§22.7 / §22.14)** — when Rachel updates the website, the KB re-verifies affected entries. Her website is the source of truth; the KB propagates.
  - **Seller Page Enrichment (§26.7.2 / §4.4.10 / §4.4.11; capability `page_enrichment`)** — Opus-tier; crawls seller's public surfaces and drafts Seller Organization Page content for review. 1/yr on Starter, 3/yr on Growth, 12/yr on Scale, unlimited on Enterprise.
  - **Capability Declarations linked to KB entries (§26.3 / §26.4 / §4.4.4)** — Rachel authors positioning once as structured declarations; buyers filter the Marketplace by them. Messaging becomes machine-readable.
...
- **Objections and rebuttals.**
  - **"AI-generated Seller Pages sound like spam."** — Every Seller Page Enrichment output is editorial-reviewed by the seller. The output is a draft for review, not an auto-publish. Verified domain ownership gates publication (§26.7.2 / §4.4.10 / §4.4.11). Unapproved drafts never surface in the Marketplace.
...
| **Apollo / ZoomInfo / LinkedIn Sales Navigator** | Contact/intent data | Large contact databases, intent-keyword triggering | Intent data is inferred from browsing signals; 90% noise. Sourcera's Seller Signals (§27.9) come from inside active structured evaluations — de-anonymized only when the buyer explicitly sends an EOI or direct invite. Converted intent, not inferred intent. | *"Apollo gives you guesses. Sourcera gives you RFPs."* |
...
|---|---|---|
| Fixed 13-phase pipeline with gate validation | Cut 6–12 week evaluations to 4–6 weeks by structure alone | Master Spec §2 / §10 |
| Agent Pre-Scoring (Sonnet, Phase 10) | Reviewers accept/override rather than scoring from scratch; ~50% scoring-phase time reduction on standard controls | §21.4 capability 10 |
| Policy-Powered Requirement Generation | 40 hours saved per evaluation on compliance translation; one upload covers every downstream evaluation | §12 |
| Append-Only Grade History + SHA-256 Selection Record | Audit-ready by construction; replaces the 10-hour PowerPoint | §13.5 / §13.10.3, §10.12 / §4.3.23 / §4.3.24 |
| Unlimited seats on every paid tier | Cross-functional collaboration without license negotiation; Legal + InfoSec + Finance + IT on every evaluation at zero marginal cost | Buyer Pricing Strategy v3 §2.1 / Master Spec §34.1.1 |
| Outcome-accounted AI pricing (accepted 10× cost, rejected 1.05× cost) | No bill shock; rejected AI outputs cost roughly $0; budget denominated in stable value-dollars | Buyer Pricing Strategy v3 §11 / Master Spec §34.3 |
| TCO Modeling as first-class Use Case | Apples-to-apples multi-year cost comparison with 6 structured pricing schemas | §15 |
| Scenario Modeling | Stress-test rankings without rebuilding the spreadsheet; 10 scenarios Business, 50 Enterprise | §14 |
| Vendor Discovery Marketplace + EOIs | Qualified vendor inbound without cold outreach | §27 |
| Real-time collaborative scoring with Disagreement Insight Cards | Eliminate "chase for scores" cycle; ≥0.3 variance auto-flags for Use Case Lead | §13.6.1, §13.6 |
| Pulse Health Score (0–100, weekly digest) | Single number for exec-sponsor status; replaces verbal updates | §20 |
| Post-Phase-13 export to Jira/Linear/Asana/Azure DevOps/Salesforce | Selection outcomes reach implementation team instead of dying in PDF | §31.3 / §32.10.3.F |
...
|---|---|---|
| KB Bootstrap (Opus-tier, Domain Bootstrap inside SSO redirect latency) | First bid responded in ~35 min instead of ~2 days; 40–200 proposed entries before first click | §48.8 / §49.1 / §22.14 |
| First-Pass RFP Response Generator with cite-before-state rule | 40–70% of requirements drafted from your own content with inline citations; ungrounded statements dropped | §22.14.1 / §24 |
| KB-gap detector on rejected drafts | Every rejection becomes a KB entry in one click; next bid reuses it | Seller Pricing Strategy v3 §15.3 / Master Spec §22.18 |
| Stake-Reveal Screen on submit | Quantified receipt of what you built: entries, citations, estimated $/yr saved | §48.8.5 / §22.18 |
| KB Value Meter (persistent on Free) | Running visibility of compounding value: citations in closed bids, win-rate lift, estimated saved time | §22.18 / §32.9 |
| Upgrade Carry-Over Guarantee | Every KB entry, confidence score, citation, and provenance record carries forward unchanged on upgrade | §22.18 / §32.9, Seller Pricing Strategy v3 §17.4 / Master Spec §22.18 |
| Honest Portability (KB exportable in full, every tier including Free) | No lock-in; Markdown + JSON + metadata + provenance + attachments, one click | §22.18 / §32.9, Seller Pricing Strategy v3 §17.1 / Master Spec §32.9 |
| Published Seller Profile on domain verification (free, every tier) | SEO-emitting Schema.org structured data from day one | §26.1 / §26.7, Master Spec §34 |
| Free $5 AI budget sized for one end-to-end first-pass-drafted bid | No paywall on the invited bid that brought you | Seller Pricing Strategy v3 §4 / Master Spec §34.1.2 |
...
| Feature | Quantifiable benefit | Source |
|---|---|---|
| Marketplace Listings + EOI Workflow | Inbound demand from buyers inside active 13-phase evaluations — not inferred intent | §27 |
| Match Scoring (Growth+, numeric) | Ranked vendor-opportunity fit; cut noise in EOI review | §27.4 |
| Seller Signals (k-anonymized buyer intent) | Pre-EOI category demand visibility routed to CRM; de-anonymizes on explicit buyer action | §27.9 |
| CRM Sync (Salesforce, HubSpot, Dynamics, Pipedrive — Growth+) | Bidirectional sync; Marketplace activity appears in the CRM your AEs already live in | §31.9 |
| Capability Declarations (machine-readable, linked to KB evidence) | Searchable, filter-matching; structured positioning | §26.3 / §26.4 / §4.4.4 |
| Verified / Certified badges (free when earned) | Trust signals lifting match-score weight; no pay-to-play | §26.2 / §34.16.2, Seller Pricing Strategy v3 §12.2 / Master Spec §34.16.2 |
| Automated Firecrawl ingestion (weekly on Starter, daily on Growth, real-time on Scale) | KB stays current with website changes automatically | §22.7 / §22.14 |
| KB-to-Capability Auto-Suggestion (§26.4 / §22.14.3) | KB entries generate declarations that feed Match Scoring and Category Pages | §26.3 / §26.4 / §4.4.4 |
| Ghost-Bid Importer (M15) | Paste one historical RFP; bootstrap your KB with prior work; first import free per Seller Org | §4.4.17 / §22.14.4 / §48.7.4 / Seller Pricing Strategy v3 §18.5 |
| Published Seller Org + SellerSoftware pages with Schema.org | Organic search compounding; per-category SEO moat | §26.7 / §26.8.5 / §4.4.10 / §4.4.11 |
| Seller Page Enrichment (Opus) | Draft rich public surfaces from your public content | §26.7.2 / §4.4.10 / §4.4.11 |
| Promoted Listings (Scale+, 1/mo Scale, 3/mo Enterprise) | Category-targeted demand capture with second-price auction and frequency caps | §27.11.2 / §34.16.1 |
...
Enterprise is gated by four explicit conditions (buyer and seller both): **SSO required, DPA required, ≥$12K/yr AI commit, or custom integration** (Buyer Pricing Strategy v3 §6 / Seller Pricing Strategy v3 §8 / Master Spec §34). Below this bar, the dedicated CSM load destroys margin — Scenario C in both pricing docs shows Enterprise below the $12K AI commit blends at ~42–44% gross margin, which is a margin floor violation.
...
> Neither does any of your peer organizations today — the category is new. Most of the first hundred Sourcera customers line-item this under either "Procurement Technology," "GRC Tooling" (when led by InfoSec), or "Vendor Management" (when led by Legal). We can help you frame the internal business case — the Time-Saved Baseline (§51.6) quantifies hours saved per evaluation, and at $100/hr blended loaded rate, Business Starter pays for itself in the first evaluation. The ROI argument is the budget-line argument.
