...
|---|---:|---|
| `runtime_active` | 278 | Proven by local detector / release artifact evidence. |
...
| `spec_binding_pending_pack_m02_3` | 13 | Product-code/static-analysis or mixed runtime rows still missing; not honestly closable from docs alone. |
...
Current blocker count: **146**.

Current JSON proof: `_audit/_tmp/v711_stamp_gate_after_phase_5_1_seller_teams_acceptance_criteria.json`.

Exact-status right-edge scan after the Phase 5.1 closure: **0 open P0, 0 open P1, 0 blocked P1, 567 open P2, 191 open P3**.

This supersedes the earlier 2026-07-09 420-row / 271-`runtime_active` snapshot. The later Phase 3.5 DSAR P2 closure added two spec-tree runtime-active rows; D-DEC-010 added one cost-base drift-threshold row; D-DEC-009 added one MCP rate-limit launch-posture row; D-DEC-011 added one KB injection scanner cadence row; D-DEC-012 added one Decisions status-schema row; and the Phase 37 no-hardcoded-directional-CSS pass added one static-analysis row. The pending-pack blocker count is now 146.
...
```bash
tools/spec-lint/node_modules/.bin/tsx tools/release/stamp_gate.ts --json > _audit/_tmp/v711_stamp_gate_after_phase_5_1_seller_teams_acceptance_criteria.json
...
- Stamp gate: FAIL on 146 runtime-evidence blockers after parsing 426 runtime rows with 278 `runtime_active` rows.
- Exact-status scan: 0 open P0, 0 open P1, 0 blocked P1, 567 open P2, 191 open P3.
