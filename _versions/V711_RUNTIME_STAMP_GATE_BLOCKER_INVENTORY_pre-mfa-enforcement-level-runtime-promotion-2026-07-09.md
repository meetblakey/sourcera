...
163 M02.3 rows have now been promoted during the 2026-07-07 through 2026-07-09 runtime-promotion passes after direct detector proof. Rows close only when runtime evidence lands and the Master Spec row is explicitly promoted to `runtime_active`.
...
| `m02_3` | 89 | Per-gate detector under tools/spec-lint/gates/<gate_id>.ts. |
...
|---|---|---|
| `webhook_payload_k_anonymity_floor` | `tools/spec-lint/gates/webhook_payload_k_anonymity_floor.ts`; pass/fail fixtures; direct gate PASS; full spec-lint PASS; stamp target absent | Promoted to `runtime_active`; §34.16.7 / §34.16.8 / Appendix C / Appendix G now have detector proof that customer-visible marketplace-discovery settlement webhooks are recipient-owned, k=5 aggregate, or Ops-only, and that loser payloads do not expose winner identity or paid amount. |
...
The one-row delta from the prior 223-blocker inventory is the §M.5.20 webhook payload k-anonymity spec-tree promotion. The pass adds a detector for §34.16.7 / §34.16.8 / Appendix C / Appendix G settlement payload privacy and removes an overbroad deploy-validator claim from the M02.3 row. This promotion proves only spec-tree payload/catalog consistency. It does not claim product emitted-payload deploy-validator proof, settlement-job runtime behavior, webhook delivery, integration tests, DLQ execution, or subscriber behavior; those remain owned by product-pack evidence. Product-code and runtime-test rows remain pending where the checked artifact class names deploy validators, integration tests, billing runtime, marketplace runtime, or other product evidence not present in this documentation corpus.
