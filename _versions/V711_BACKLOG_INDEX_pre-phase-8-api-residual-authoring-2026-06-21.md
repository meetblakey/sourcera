...
| Parsed canonical P1-open rows after 2026-06-21 closeouts | 600 rows currently parse as `severity=P1` and `status=open` in `_audit/DEFECT_LEDGER.md` | Advisory only. This does not replace the 808 headline until D-CONS latest-status propagation, duplicate review, and cluster-count reconciliation close. |
...
**2026-06-21 — Phase 8 Prompt 8.1 API scope sync.** `BL-P1-PH8P81-API` now carries 3 live API-authoring residuals: D-V8.1-002, D-V8.1-006, and D-V8.1-008. D-V8.1-003 / D-V8.1-004 / D-V8.1-005 are closed by §32.5 Vendor Opt-Outs + Marketplace Discovery catalog registration. D-V8.1-015 is closed by syncing §32.5 / §32.6.1 to the already-authored §25.3.10a Disqualification Reversal contract. Verification records: `_audit/PHASE_V72REM_PHASE_8_API_CATALOG_EXISTING_SURFACES_VERIFY.md` and `_audit/PHASE_V72REM_PHASE_8_DISQUALIFICATION_REVERSAL_API_SYNC_VERIFY.md`.
