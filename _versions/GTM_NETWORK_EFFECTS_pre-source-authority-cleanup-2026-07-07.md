...
**Companion docs:** `Sourcera_Master_Summary.md` §3, `Sourcera_Seller_Pricing_Strategy.md` §14–18, `GTM_POSITIONING.md` §1.2–1.3, §4, §5.2–5.4
**Last updated:** 2026-04-20

This document is not a re-derivation of the ten growth loops, seventeen growth mechanics, or seven seller-side network effects. Those live canonically in `Sourcera_Master_Summary.md` §3. This document is the operational execution plan for building both sides of the two-sided market as a solo operator during the first twelve months: which mechanisms must be live before which, which surfaces the founder manually operates before automation takes over, what conversion moments the product must catch, and what metrics prove the flywheel is turning.

Everything here respects the constitutional constraints of the platform: vendor participation is always free, the Marketplace is opt-in and summary-only, published profiles are free from day one, Promoted Listings are Scale/Enterprise only, and the anti-spam framework in §3.5 is the acceptable-use floor regardless of plan.

---

## Section 1 — Network Effects Inventory: Operational Status

The sections below treat each Growth Loop (from `Sourcera_Master_Summary.md` §3.2), each Growth Mechanic (§3.3 M1–M17), and each Seller-Side Network Effect (§3.4) as an operational system. For each: the product surface that must exist for the loop to fire, the loop time and branching factor (best estimate given the product model, not yet empirically measured), the solo-operator intervention that accelerates it, the priority at launch vs. post-launch, and the kill condition — the single product defect or policy decision that would stop the loop from functioning.

### 1.1 The Ten Core Growth Loops (§3.2)

#### Loop 1 — Vendor-Invite-Creates-Account (M1)

| Dimension | Operational state |
|---|---|
| Product dependency | Magic-link SSO orchestrator that starts KB Bootstrap inside SSO-redirect latency (§2.11 requirement 13); Domain Bootstrap (§3.6); First-Pass RFP Generator (§6.14.2); Inline-citation UI; Landing counter; KB-gap detector (§3.6 Pre-arrival through In-workspace) |
| Loop time | ~0 days (fires at invite send). Time-to-next-loop-iteration = time-to-next-buyer-invite from the same or different vendor. First-invite → first-bid-submit targets p50 < 20 min, p90 < 60 min (§3.1b). |
| Branching factor | 3–10 Seller Orgs seeded per buyer evaluation (§3.2). Compounds because invited sellers themselves become invitable again by any other buyer. |
...
| Dimension | Operational state |
|---|---|
| Product dependency | Template Library with clone tracking (§6.10); Clone-attribution badge rendered on cloned template; deep-link to originating Seller's Software Page (§6.15.5). |
...
| Dimension | Operational state |
|---|---|
| Product dependency | Published Seller Profile default-on at Basic verification (§6.15.1); Software Pages (§6.15.5); Schema.org Organization + SoftwareApplication structured data emission; Seller Page Enrichment surface (paid, Starter+); sitemap submission; canonical URL structure for `sourcera.com/sellers/:slug` and `sourcera.com/software/:slug`. |
...
| Dimension | Operational state |
|---|---|
| Product dependency | AI Wallet service (§2.11); Budget counter visible in every workspace; Outcome Resolver with per-capability signals (§2.9); 10-free-per-capability allowance (Appendix C.80); Contextual upgrade CTA at exhaustion with ROI inline ("$X of value delivered; upgrade to continue"). |
...
| Dimension | Operational state |
|---|---|
| Product dependency | Command Palette (`Cmd+K`) with context-scoped results (§6.21); Marketplace content index; phase-aware result ranking; deep-link routing into Seller Pages, Software Pages, Category Pages. |
...
| Dimension | Operational state |
|---|---|
| Product dependency | Bid close workflow (§6.14.5 equivalent); Phase 13 Vendor Response KB Sync offer (§6.13.1 closed-bid ingestion channel); Win/Loss debrief surface (§3.1b step 7); one-click "save to KB" CTA. |
...
| Dimension | Operational state |
|---|---|
| Product dependency | Template authoring inside Seller Console; Visibility Credits mechanic (featured placements in Category Pages earned by clone volume); Template Spam ML Classifier (§3.5) to prevent abuse; clone-volume dashboard visible to publishing Seller. |
...
| Dimension | Operational state |
|---|---|
| Product dependency | Qualitative Match Score labels visible to Free/Starter sellers ("Strong match"); numeric scores paywalled behind Growth+ (§6.16.4); EOI workflow (§6.16.2) as the downstream conversion surface. |
...
| Branching factor | Not a branching loop — a conversion mechanic. Drives the "ambition" conversion moment (§3.1b conversion moment #2). |
...
### 1.2 The Growth Mechanics M1–M17 (§3.3)
...
|---|---|---|---|---|---|
| **M1 Vendor-Invite-Creates-Account** | Magic-link forced signup with Hero Moment | §3.6 full stack | **Launch** | Monitor forced-signup funnel daily; triage bootstrap failures same-day | Hero Moment async-fails |
| **M2 Selection Report Public Link** | Watermarked shareable selection artifact with per-section redaction | Selection Report (§6.37); public-link generator with redaction controls | **Month 3** | Personally draft the watermark + redaction default policy; seed-share the first 5 customer-approved reports on LinkedIn | Buyers never publish (default redaction too restrictive or too loose) |
...
| **M5 Buyer-Pull Vendor Invite** | Anonymous templated outreach to vendors not yet on Sourcera | Invite email template system; Vendor Opt-Out Registry (C.123); DMARC/SPF reputation | **Launch** | Review every templated outbound email before send for the first 500 invites; build the reply-handling playbook | Low deliverability; opt-outs spike; reputation suspension |
...
| **M8 Org Intelligence Value Curve** | Compound analytical value per additional evaluation, visible in-app | Org Intelligence dashboard (§6.7) with evaluation-count milestones | **Month 6–9** | None direct — data accumulates | Org runs < 3 evaluations (never hits the curve); value is invisible |
...
### 1.3 The Seven Seller-Side Network Effects (§3.4)
...
- **Surface dependency.** Domain-Bootstrap; Default-on profile publication at Basic verification (§6.15.1 Published-from-Day-One Default); SellerSoftware entity creation on signup; Marketplace Tagger (auto-categorization from domain + buyer evaluation context).
...
- **Surface dependency.** KB-to-Capability Auto-Suggestion (C.46); Controlled Marketplace Taxonomy (C.60); Numeric Match Scoring (Growth+, §6.16.4); EOI Workflow (§6.16.2); Category Pages (M9); Comparison Pages (M11); Market Intelligence Reports (M12).
...
- **Kill condition.** Auto-declarations are low-quality and get rejected at high rate — the capability underperforms economically (per §2.9, rejected ops bill at cost, so a persistently-rejected capability is a margin leak).

#### Network Effect 3 — Bid-Close → Win/Loss Signal → Platform Learning

- **Surface dependency.** Phase 13 closure signal; buyer-side confirmation of winner; Win/Loss debrief generation (§3.1b step 7); citation-graph tracking per KB entry; Outcome Resolver capturing accepted signals.
...
- **Surface dependency.** Ghost-RFP Ingestion capability (Opus, $25 value per RFP, §2.8 rate card); first import free per Seller Org.
...
The direct-outreach pitch is structured around the §6.15.1 Published-from-Day-One default, inverted from a cold pitch into an opt-out offer:
...
A Seller who completes DNS verification and clicks "publish my profile" is eligible for their lifetime-free KB Bootstrap (§6.13.6) inside the same session. The founder's post-signup playbook:
...
| Forced-signup sellers experiencing Hero Moment | 40+ | Target p90 Hero Moment success rate (§3.1b leading indicators) |
...
- 80–90% of forced signups reach the Hero Moment (per §3.1b leading indicators — bootstrap success rate target).
...
The three conversion moments (§3.1b, §14.3 Seller Pricing) fire at structurally different times across the funnel:
...
1. **Minutes from magic-link to first submitted requirement response** (p50, p90). Target p50 < 20min, p90 < 60min (§3.1b). Degradation = Hero Moment regression, triage immediately.
...
Forced signups are structurally the best supply-acquisition channel in the entire GTM. The vendor arrives at peak pain, with a deadline, an incoming RFP, and no workflow to defend (§3.1b). Every product and operational decision should maximize yield here.

#### 3.1.1 Maximizing Vendors Per Buyer Evaluation

**Coaching.** During founder-led onboarding of the first 30 Buyer Orgs, the founder actively coaches the buyer on shortlist breadth: *"Invite 7–9 vendors, not 3–4. The 13-phase pipeline is designed to scale up to 10. You can always disqualify in Phase 4–5 via Vendor Voluntary Withdrawal (C.5). Inviting more vendors costs you nothing and the evaluation quality rises with comparative depth."*

**Invite suggestions.** The Agent's Vendor Suggestion capability (reference: Master Summary §6.12.1 vendor-suggestion capability, also referenced as M5 Buyer-Pull Vendor Invite at anonymous level) should surface Marketplace-matched vendors during Phase 4–5 Vendor Discovery, tuned to the buyer's Capability Declarations filter. A buyer who types three vendor names should see 4–6 more Sourcera-matched suggestions to round out the shortlist.
...
**Progress storytelling integrity.** Every line of onboarding copy must be true and reflect actual work (§3.6 Onboarding surface). Fabricating progress states (common B2B onboarding dark pattern) kills trust in the single moment that matters most.

**Banned anti-patterns** (§3.6, §16.3 Seller Pricing).
...
- **RevOps fourth.** Dense competitive landscape; CRM Sync (§6.18) + Seller Signals are the differentiator vs. Responsive; mature buyer market.
...
- **Day 7:** Automated reminder with KB-to-Capability auto-suggest link (C.46 — uses the Haiku-tier AI capability, cheap and high-volume).
...
The lifetime-free KB Bootstrap (§2.1, §6.13.6) is the highest-leverage activation moment a seller experiences. For organic-signup sellers (not forced-signups who get automatic bootstrap), this is a manual trigger inside the Seller Console.
...
Template publishing is a lower-frequency, higher-leverage action than Capability Declaration authoring. The incentive is editorial-featured placement in Category Pages (§2.10 Featured Placements are strictly editorial, not purchasable — this is the carrot).
...
- **Ongoing.** Once Templates hit scale, the Template Spam ML Classifier (§3.5) gates quality; the founder only manually approves cross-category or high-visibility Template placements.
...
**Thesis.** Every buyer evaluation is a supply-acquisition event. Spending founder time on buyer acquisition is structurally spending founder time on supply acquisition — because Loop 1 (§3.2) fires inside every evaluation.
...
- Shared methodology (the Sourcera Method, §4 Master Summary)
...
**Mechanic.** Verification tiers (Basic / Verified / Certified, per §2.10 and §6.15.2) are never a billing surface. They are earned trust signals. The strategy is to **make earning them feel achievable and time-bound**, driving sellers to invest in profile completeness and documentation upload.
...
3. **Controlled Taxonomy review.** Capability Declaration taxonomy (C.60) reviewed for the new category — any gaps closed with new tags.
...
Per §6.16.5, M9 and M11 are refreshed on cadence by Sourcera-owned AI capabilities (`category_faq_generation`, `guide_refresh_analysis`, etc.). Auto-compounding means:
...
1. **Evaluation history.** Past evaluations, scoring decisions, Selection Records are retained on-platform. Moving off Sourcera means re-authoring every historical evaluation from exports (§6.33) — technically possible, operationally expensive.
...
3. **Organizational Intelligence (§6.7).** Cross-evaluation analytics (vendor history, scoring inconsistencies, efficiency metrics) are derivative of on-platform data. Moving off loses the compounding intelligence surface entirely.
...
1. **KB confidence scores, citation graph, win-rate weights** (§6.13.7 On-Platform Compounding). These compound learnings are not portable — they are Sourcera's learning from seller behavior, not raw KB content. Export preserves the text; the intelligence stays behind.
...
**The honest-portability commitment (§6.13.7).** KB is exportable at every tier, including Free. This is strategic. It builds trust (reducing the "lock-in" objection that kills B2B deals in procurement-heavy orgs), which in turn makes paid conversion easier. Stake is built from compounding utility, not hostage-taking. The seller can always leave with their text KB; what they can't take is the compounding on-platform intelligence. That's the moat.

### 7.3 The Data Moat — Outcome-Based AI Accounting

**Mechanism.** Every AI Operation settles as `accepted` or `rejected` per a capability-specific signal (§2.9). This signal is a proprietary data asset with three compounding properties:
...
3. **Cross-vendor Match Score calibration.** Match Score accuracy improves with every EOI-view-to-EOI-submit outcome (§2.9 Match Score accepted signal). Cross-vendor learning compounds as Marketplace inventory grows.
...
- Controlled Capability Taxonomy (C.60) — an editorial asset that requires category-by-category curation
...
- Each successful evaluation on Sourcera is an implicit endorsement of the methodology (the Sourcera Method, §4 Master Summary) that reduces buyer acquisition friction for the next buyer.
...
| Network Effect 2 | Capability Declaration auto-suggest acceptance rate <20% | Taxonomy review (C.60); may be missing category coverage |
...
| M1 forced-signup | Invite email deliverability <85% | DMARC/SPF reputation check; Suppression List audit (§3.5); pause outbound if reputation damaged |
