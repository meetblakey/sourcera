...
169 M02.3 rows have now been promoted during the 2026-07-07 through 2026-07-09 runtime-promotion passes after direct detector proof. Rows close only when runtime evidence lands and the Master Spec row is explicitly promoted to `runtime_active`.
...
|---|---:|
| `runtime_active` | 202 |
| `spec_binding_pending_pack_m02_3` | 83 |
...
| `m02_3` | 83 | Per-gate detector under tools/spec-lint/gates/<gate_id>.ts. |
...
|---|---|---|
| `conversion_funnel_registry_canonical_consumer` | `tools/spec-lint/gates/conversion_funnel_registry_canonical_consumer.ts`; pass/fail fixtures; direct gate PASS; TypeScript PASS; full spec-lint PASS; stamp target absent | Promoted to `runtime_active`; §51.0.3, Appendix G, and §51 dashboard consumers now have spec-tree proof that every customer-facing funnel uses the closed registry. |
...
| M.5.12 V13 catalog additions (PLG / Growth / Analytics / Hero Moment) | 12 |
...
The one-row delta from the prior 217-blocker inventory is the §M.5.12 `conversion_funnel_registry_canonical_consumer` spec-tree promotion. The pass proves only the documentation registry / catalog / dashboard-consumer contract: §51.0.3 uses Appendix G event names, §51.3 / §51.5 consume the registry instead of inline stage lists, and Appendix G registers the referenced funnel events. It does not claim PostHog API auto-generation, production query generation, product analytics delivery, deploy validators, integration tests, or runtime funnel correctness; those remain owned by product-pack/runtime evidence.
