...
| Parsed canonical P1-open rows after 2026-06-21 closeouts | 597 rows currently parse as `severity=P1` and `status=open` in `_audit/DEFECT_LEDGER.md` | Advisory only. This does not replace the 808 headline until D-CONS latest-status propagation, duplicate review, and cluster-count reconciliation close. |
