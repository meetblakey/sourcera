...
**Version:** 1.0
**Date:** 2026-04-14
**Source Documents:** `Sourcera_Master_Spec.md` v6.0.0  · `KB_Engineering_Spec.md` · `UX_Design_of_Sourcera.md` · `Sourcera_Buyer_Pricing_Strategy.md` · `Sourcera_Seller_Pricing_Strategy.md`
**Purpose:** Single-document reference that compresses the full Sourcera surface — positioning, method, design language, pricing, PLG, and every feature and sub-feature — into a buildable, sellable, and reviewable overview.
...
### Strategic Principles (Both Sides)
...
- **Invited participation is free.** A vendor invited to a buyer's evaluation can always respond in full, for free, to that invitation. Proactive discovery, KB automation beyond the lifetime-free bootstrap, outbound Marketplace activity, and revenue-ops integrations are monetized.

### Buyer Plan Architecture
...
Vendor response to an invited evaluation is **always free**. Published Seller Profile is free from day one — this is the mechanism by which the supply side of the Marketplace populates organically with every buyer evaluation.

### Cross-Side Billing and AI Budget Pooling

An Organization may run a Buyer plan, a Seller plan, or both. Plans activate per console. AI budgets are Org-scoped and pooled across consoles — if an Org holds Buyer Growth ($300 budget) and Seller Starter ($30 budget), it has $330 of unified AI budget spendable by any capability in either console. Wallet overage is Org-scoped (one wallet, one cap). The Dual-Console Firewall is unchanged — billing is shared; data is not. Enterprise collapses into one contract when both sides apply, with a single committed spend minimum at the higher floor ($3K/mo).

### Gating and Upgrade Path
...
- **Free → Starter (Seller):** second concurrent invited bid, first proactive EOI, KB 50-entry ceiling, or Firecrawl request.
...
### Pricing Engineering Requirements

The platform must ship the following to run the model: AI Wallet service (Org-scoped, pooled across consoles), Outcome Resolver (per-capability signal contracts), nightly cost_base recalculation job, spend cap enforcement (hard block), budget notifications at 50/80/100%, Usage Dashboard (per-capability spend, acceptance rate, top consumers, trend), payment gating on Opus-tier ops, downgrade-safe 90-day read-only preservation, audit log of billing events, cross-console plan-assignment model, Pro Trial Seat allocation pool and 30-day auto-downgrade, and a public rate card at `api.sourcera.com/v1/pricing`.

### Financial Target

Blended gross margin ≥ 90%. Target Year-1 exit mixes:
- **Buyer side:** 60% Starter / 25% Growth / 10% Scale / 5% Enterprise
- **Seller side:** 55% Starter / 25% Growth / 15% Scale / 5% Enterprise

Enterprise is deliberately kept < 10% of accounts on both sides until the CSM motion is templatized.
...
4. **Expansion (Starter → Growth → Scale).** Active evaluations and AI budget utilization drive tier moves.
...
**Related (Appendix C) for Seller KB (6.13 overall):** C.46 KB-to-Capability Auto-Suggestion · C.52 KB MCP Server · C.53 KB Retrieval Pipeline · C.54 KB Indexing Pipeline · C.55 Managed Agent Definitions · C.56 Custom Tool Contracts · C.57 Skills · C.58 Environments · C.59 Beta Header & Version Pinning.
