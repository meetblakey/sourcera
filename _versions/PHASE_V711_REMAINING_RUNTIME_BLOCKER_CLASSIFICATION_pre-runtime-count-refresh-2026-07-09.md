...
|---|---:|---|
| `runtime_active` | 271 | Proven by local detector / release artifact evidence. |
...
```bash
tools/spec-lint/node_modules/.bin/tsx tools/release/stamp_gate.ts --json > _audit/_tmp/v711_stamp_gate_after_source_authority_bookkeeping.json
npm --prefix tools/spec-lint run all -- --no-emit --ae-ledger ../../_integration/AUTHORED_EXTENSIONS_LEDGER.md
```

Results:

- Stamp gate: FAIL on 147 runtime-evidence blockers.
