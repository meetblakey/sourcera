...
**v3 changeset:** Added the **Seller Solo tier** ($49/mo annual / $59/mo monthly OR $199/bid one-time) as the personal-card on-ramp between Seller Free and Seller Starter — the surface for **Seller Maya**, the first-time bid responder (Account Executive, Solutions Engineer, or Bid Manager at a 50–500-person B2B SaaS vendor) introduced as the v7.1.0 seller-side surface persona. AI consumption is invisible to Solo-tier sellers (cost absorbed within a $5/mo or $5/bid value envelope; engine throttles silently if approached — no $5 hard-cap bill-shock). Solo unlocks 250-entry KB ceiling, Verified-tier eligibility, 1 weekly Firecrawl source, and email support over Free, while preserving 1 concurrent bid and 0 outbound EOIs to keep the Starter conversion lane clear. Updated plan-architecture table, §1 core principle, §2 strategic decisions (added §2.8 and §2.9), PLG paid path (now Free → Solo → Starter → Growth → Scale → Enterprise), Financial Scenarios (added Solo scenario D), and Migration delta. All numerical limits remain authoritative in this document until Master Spec §34.1, §5.11, §39, and Appendix J absorb them under the v7.1 integration program (Phase 14.9 — Master Spec scaffolding edit pending). Section numbering shifted: prior §5–§21 are now §6–§22 to accommodate the new §5 Seller Solo Plan.
...
| **Engine-absorbed AI on a single active bid (no $5 hard cap, silent throttling)** *(new in v3 — Solo)* | **Paid (Solo on-ramp at $49/mo or $199/bid)** |
...
**Decision: Never charge to respond to an invited bid.** Even the First-Pass Responder, when used inside an invited bid, spends from the seller's AI budget — it does not block response altogether. A Free-tier seller can always respond manually to an invited bid and use the free $5 AI allowance for drafts, Q&A, and suggestions. **Solo lifts the $5 hard cap to a silently-absorbed envelope** so the personal-card seller never sees a mid-bid "out of budget" wall.
...
This decision is materially important on the seller side because the Free-tier $5 hard cap can fire mid-bid on dense RFPs (a single First-Pass draft for an 80-requirement RFP at 70% accept costs ~$5.55 — the Free budget is sized to cover a small bid only). Solo's silent throttling is the first paid surface where Seller Maya's bid response cannot be derailed by an AI-budget wall mid-flight.
...
| AI budget (value $) | $5/mo (hard cap) | **Hidden ($5 absorbed; silent throttling)** | $30/mo | $200/mo | $600/mo | Committed |
...
| First-Pass RFP Generator | Budget only ($5 hard cap) | **Engine-absorbed (silent)** | Included (visible budget) | Included (visible budget) | Included (visible budget) | Committed |
...
**Not included:**
- Engine-absorbed AI envelope (Solo+) — Free's $5 hard cap can stop a bid mid-response
...
- **Engine-absorbed AI envelope** ($5 value/mo subscription or $5 per bid one-time). No hard cap; engine throttles low-priority capabilities silently if envelope is approached. Seller never sees value-dollars, wallet UI, rate card, or per-op pricing.
...
2. **AI budgets are Org-scoped and pooled across consoles — at Business+/Starter+ tiers.** If an Org has Buyer Growth ($300 budget) and Seller Starter ($30 budget), it has $330 of unified AI budget. Any capability in either console draws from it. **Solo-tier absorbed envelopes do not pool** — each Solo console maintains its own absorbed envelope, surfaced as a single line item per console. An Org holding Buyer Solo + Seller Solo has two $5/mo envelopes (one per console), engine-managed independently.
...
- AI COGS:
  - Subscription: 300 × $0.54/mo absorbed (full $5 envelope at 70/30 mix) × 12 = $1,944
...
|---|---|---|---|---|
| **1. Mid-bid AI budget wall** *(new in v3 — primary Solo trigger)* | Vendor's $5 Free hard cap exhausts mid-response on a dense RFP | Necessity — *"my response is blocked"* | **Solo** | *"Your bid response is paused — Sourcera Free's AI budget is exhausted. Solo unlocks the engine-absorbed envelope (no hard cap, no mid-bid walls) for $49/mo or $199/bid."* |
...
- **% of Free sellers who hit the $5 AI budget wall mid-bid** *(new v3 — drives Solo conversion)*
...
**Live budget counter (Free only).** Top of workspace shows *"AI budget used: $1.47 of $5.00 · 23 drafts accepted, 3 rejected."* Cost is visible but not alarming — the budget is sized to cover exactly one small bid end-to-end. **On Solo, this counter is hidden** (per §2.9); the seller sees only "Plan: Solo" with no budget anxiety surface.
...
Registered as growth mechanic M17 in the Master Summary §3.3 catalog.
...
| Solo AI budget behavior | — | **Hidden ($5 absorbed; engine throttles silently — no hard cap)** |
...
- Master Spec §10.8 (plan tiers) — register Seller Solo
- Master Spec §10.7 / §21.4–21.5 (capability catalog) — confirm Solo absorbed-envelope routing
- Master Spec §22 (Seller KB integrated portions) — add Solo Verified eligibility, 250 KB cap, 1 weekly Firecrawl, 1/yr re-bootstrap notes
- Master Spec §34 (billing) — extend cross-side billing rules to clarify Solo non-pooling
...
- Master Spec **§34.X (new) — Solo-tier billing surface** (shared with buyer-side; per-console line item)
- Master Spec **§34.Y (new) — Per-bid charge orchestration** (Stripe trigger on bid submission; 7-day refund window with buyer-open check; 90-day per-bid KB retention)
- Master Spec §39 (object size constraints) — add Seller Solo row (250 KB entries, etc.)
- Master Spec **§44.X (new) — Solo-Tier Surface Treatment** (engine-only AI consumption + silent throttling; shared with buyer-side)
- Master Spec §5.11 (Feature Access Matrix) — Seller Solo row across all features
- Master Spec Appendix J — confirm `solo` plan tier value (registered in buyer-side migration; same enum value used cross-console)
- Master Spec Appendix M — add rows for Solo billing surface, Solo Verified eligibility, Solo Capability Declaration auto-generation, Solo silent throttling
- `KB_Engineering_Spec.md` §13.2 — confirm `ai_operation_pricing` table includes all seller-side capabilities listed in §10 above
