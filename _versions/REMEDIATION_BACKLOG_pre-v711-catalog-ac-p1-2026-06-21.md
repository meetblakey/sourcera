...
|---:|---|---|---|---:|---|---|---|---|---|---|---|---|---|
| 1 | BL-P1-PHSS-ACC | Phase SS — UI Surface State Coverage | acceptance_criteria | 39 | D-SS-001…005 (+34 more) | §3 / §13 / §14 / §17 / §20 / §22 / §27 / §50 | UI surface state coverage missing testable AC blocks across 39 surfaces. | Author numbered ACs per §13.10 style for each surface state (loading / empty / error / partial). | Engineering | M24.3 | XL | PROD-281 | AE rows for each multi-section term (~6 expected) |
| 2 | BL-P1-PH22-ENUM | Phase 2.2 | enum | 24 | D-2.2-001…005 (+19 more) | §4.4 series + §22 + §27 | 24 P1 enum values referenced inline without Appendix J registration. | Bulk Appendix J registration pass; cite source location in each row. | Engineering | M02.3 | L | PROD-010 | Composite AE row for 24-value Appendix J expansion |
| 3 | BL-P1-PH32-PLAN | Phase 3.2 | plan_gating | 20 | D-3.2-001…005 (+15 more) | §5.11 + §34.1 + §39 | 20 plan-gated features lack §5.11 / §34.1 / §39 row triplets. | Add Feature Access Matrix row + plan-tier row + size constraint row per feature; cite tables; no inline literals. | Engineering | release-orchestration | L | PROD-028 | New AE row "Plan-gating matrix completeness pass" |
| 4 | BL-P1-PH31-RBAC | Phase 3.1 | rbac | 13 | D-3.1-001…005 (+8 more) | §5 + §50.3 | 13 §5.11 Feature Access Matrix rows missing role × action × column. | Author missing matrix rows; bind every role to a §50.3 canonical role enum value. | Engineering | M11.3 | L | PROD-027 | AE row for Feature Access Matrix completeness |
...
| 6 | BL-P1-PH8P81-WH | Phase 8 — Prompt 8.1 — §32 API | webhook | 13 | D-V8.1-006…010 (+8 more) | §31 + Appendix C + Appendix G | 13 webhooks missing HMAC / event_id idempotency / backoff / DLQ / payload cap / Appendix C/G registration. | Author per §31; register in Appendix C; mirror in Appendix G; default retry curve binding. | Engineering | M02.3 | L | PROD-267 | Composite AE row "§31 webhook completeness pack" |
...
| 8 | BL-P1-PH8P83-NOTIF | Phase 8 — Prompt 8.3 — Appendix C | notification | 10 | D-8.3-001…005 (+5 more) | Appendix C + §29 | 10 webhook events lack Appendix C notification-event-catalog registration. | Append Appendix C rows with channel × role × plan-gating matrix; link to retry-curve binding. | Engineering | M02.3 | M | PROD-269 | Composite AE row "Appendix C completeness expansion" |
| 9 | BL-P1-PH33-DOC | Phase 3.3 | documentation_gap | 9 | D-3.3-001…005 (+4 more) | §5 / §6 / §7 narrative | 9 documentation gaps in RBAC / auth / PII handling narrative. | Author missing prose; cite Master Spec authoritative source-of-truth. | Engineering | — | M | DOC-006 | — |
...
|---:|---|---|---|---|
| 8 | BL-P1-PH8P83-NOTIF | `D-8.3-001…005` | **`D-V8.3-002, -003, -004, -005, -007 …`** (14 P1 in family) | Dropped "V"; cluster is real + open (genuine Phase 11 work). |
...
**F-3 cross-reference (not a §3.1 edit).** BL-P1-PH32-PLAN (rank 3) is now 15 `remediated` + 3 `partially_remediated` (AE-3.2-001/-002/-003 gated) + 1 open (D-3.2-012) + 1 flagged (D-3.2-014 malformed); see `DEFECT_LEDGER.md` F-3 block. The cluster's true residual execution surface is D-3.2-012 plus the §5.11 27-location inline tier-list audit (Phase 14.9.2), **not** the full 20.

**Net effect on the Phase 11 execution surface.** 48 backed clusters; 6 clusters fully de-scoped (F-2, V12-closed); BL-P1-PH32-PLAN reduced to a 1-defect residual (F-3); 2 clusters unbacked (F-5, rows 45/49). The genuine remaining Phase 11 authoring surface is the open P1 in the ~40 remaining backed clusters (re-derive exact IDs per the F-5 corrections before execution).
...
| # | Defect | Class | Subject | Owner | Target |
|---|---|---|---|---|---|
| 1 | D-6.1-002 | enum | `console_bridge_event_kind` registration in Appendix J (18 values) | Engineering | v7.1.1 |
| 2 | D-6.1-003 | enum | 4 Console Bridge Event enums in Appendix J (`event_direction`, `sync_status`, `failure_reason`, `conflict_resolution`) | Engineering | v7.1.1 |
| 3 | D-6.1-004 | enum | 5 Vendor Disqualification enums in Appendix J (`severity`, `trigger`, `notification_style`, `appeal_status`, `reversal_reason`) | Engineering | v7.1.1 |
| 4 | D-6.1-005 | webhook | `vendor.disqualified.org_level` registration in Appendix C + Appendix G | Engineering + Analytics | v7.1.1 |
| 5 | D-6.1-006 | webhook | `console_bridge.dlq_entered` registration in Appendix C | Engineering | v7.1.1 |
| 6 | D-6.1-007 | webhook | `console_bridge.reconciliation_summary` registration in Appendix C | Engineering | v7.1.1 |
| 7 | D-6.1-008 | webhook | `seller.bid_response.amendment_needs_reverification` registration in Appendix C; 7 Bid-Response-Materialization PostHog events in Appendix G | Engineering + Analytics | v7.1.1 |
| 8 | D-6.1-009 | error_code | 8 vendor disqualification reversal codes in Appendix I | Engineering + QA | v7.1.1 |
| 9 | D-6.1-010 | error_code | `vendor_disqualification_global_ban_requires_dual_signoff` in Appendix I | Engineering + QA | v7.1.1 |
| 10 | D-6.1-011 | error_code | 9 Console Bridge / Materialization error codes in Appendix I | Engineering + QA | v7.1.1 |
| 11 | D-6.1-012 | webhook | Resolve `console_bridge_standard` retry-curve binding (rename to `bridge_apply_standard` non-webhook curve in §25.2.2; route Bridge-domain webhooks to F.1 standard) | Engineering | v7.1.1 |
| 12 | D-6.1-019 | enum | `disqualification_notification_failure_reason` in Appendix J | Engineering | v7.1.1 |
| 13 | D-6.2-003 | notification | ~36 §27 webhook events in Appendix C | Engineering + Marketplace | v7.1.1 |
| 14 | D-6.2-004 | posthog_event | Same ~36 webhook mirrors + ~14 observability-only events in Appendix G | Engineering + Analytics + Marketplace | v7.1.1 |
| 15 | D-6.2-005 | enum | 13 §27 enums in Appendix J + 51 §27 error codes in Appendix I | Engineering + Marketplace + QA | v7.1.1 |
| 16 | D-6.2-007 | state_machine | 10 §27 state machines in Appendix L (or explicit cross-references per APX-L preamble) | Engineering + Marketplace | v7.1.1 |
| 17 | D-6.2-009 | surface_engine_mapping | 5 §27 surfaces in Appendix M (Hide Sponsored, Public Transparency Report, Pre-flight Scope Report, Featured Lane visual separation, Bid Window modal) | Engineering + Design + Marketplace | v7.1.1 |
...
|---|---|---|---|---|
| 18 | D-6.2-006 | data_model | Author 3 forward-referenced entities in §4.5.x: `MarketplaceMatchFeatureRegistry`, `MarketplaceMatchScoreModelVersion`, `MarketplaceMatchScoreSnapshot` at full §4.4-style fidelity. Mark as Authored Extensions and append rows to `AUTHORED_EXTENSIONS_LEDGER.md`. | Engineering |
...
| # | Defect | Class | Subject | Owner |
|---|---|---|---|---|
| 20 | D-6.1-015 | firewall_leakage | Replace `source_entity_version` on seller projection with opaque `bridge_source_handle_token = HMAC(source_entity_id ‖ source_entity_version ‖ bid_workspace_id, webhook_secret)` — same pattern as AE-PH6R-001. | Security + Engineering |
...
| 22 | D-6.1-017 | observability | Author §4.7.1.2 "AuditEvent Emission per Bridge Transition"; new CI gate `bridge_event_audit_emission_completeness`. | Engineering |
| 23 | D-6.1-020 | mobile_divergence | Author mobile-parity rows in §25.2.3 (Bridge / Sync Health panels) and §25.3.8 (reversal banner) per §38 Feature Parity Matrix. | Design + Engineering |
| 24 | D-6.2-012 | consistency_drift | Update §27.9.5 / §27.9.5.1 / §27.9.6 / §27.9.12 / §27.9.13 to use canonical `tuple_distinctiveness_threshold_exceeded`; eliminate `distinctiveness_veto` aliasing. | Engineering |
| 25 | D-6.2-013 | enum | Add `recompute_deadline_exceeded` to Appendix J `seller_signal_suppressed_reason` (rolls under §6.1 sweep #15). | Engineering |
| 26 | D-6.2-014 | enum | Author Appendix J `seller_signal_k_anon_state` (11 values); update §27.9.5 / §27.9.5.1 to use the enum. | Engineering |
| 27 | D-6.2-016 | numerical_singleton | §27.9.6.2 inline restatement of digest cadence — replace with citation to §34.1.2 cell **Seller Signals**; add deploy-time validator `seller_signal_cadence_single_source`. | Engineering |
| 28 | D-6.2-017 | enum | Confirm `webhook_delivery_audience_scope` enum value-set parity between §27.8.9 and §27.10.7 consumers. | Engineering |
| 29 | D-6.2-018 | mobile_divergence | Author §27.4.6 mobile-rendering subsection or §27.4.10 AC #6 mandating mobile parity. | Design |
| 30 | D-6.2-019 | notification | Add `marketplace_discovery.frequency_cap_bypass_suspected` to §27.11.7 webhook catalog (rolls under §6.1 sweep #13). | Engineering |
| 31 | D-6.2-020 | data_model | Read §4.4.21 in full and verify `secondary_reviewer_user_id` field exists; if absent, add it as Authored Extension; add `verification_two_reviewer_required` to Appendix I. | Engineering |
...
| 34 | D-6.1-022 | documentation_gap | Land §5.11 update for "Disqualification — All Tiers" core-workflow row | Engineering |
...
| 36 | D-6.2-008 | glossary | 13 Appendix K entries for §27 multi-section terms | Engineering |
...
| 38 | D-6.2-023 | consistency_drift | Choose canonical name "Score-Drift & Fairness Dashboard" and propagate to APX-M | Engineering |
| 39 | D-6.2-024 | consistency_drift | Normalize §27.11.7 endpoint paths to `/api/v1/...` per §32 canonical | Engineering |
| 40 | D-6.2-025 | glossary_canonicality | Appendix K disambiguation note for `seller_marketing_editor` vs `ops_marketing_editor` | Engineering |
...
| # | Gate | Pack | Owner | Target |
|---|---|---|---|---|
| 1 | `console_bridge_seller_projection_no_cardinality_leak` | M02.3 (Console Bridge runtime) | Engineering — Console Bridge / Spec Lint | v7.1.1 |
| 2 | `taxonomy_webhook_no_ops_user_identity_leak` | M21.3 (Marketplace) | Engineering — Marketplace | v7.1.1 |
| 3 | `abuse_report_webhook_no_ops_user_identity_leak` | M21.3 | Engineering — Marketplace | v7.1.1 |
| 4 | `legal_process_non_disclosure_reporter_audience_suppression` | M21.3 | Engineering — Marketplace + Legal | v7.1.1 |
| 5 | `legal_process_non_disclosure_reporter_deferred_notification_temporal_decoupling` | M21.3 | Engineering — Marketplace + Legal | v7.1.1 |
| 6 | `marketplace_abuse_sla_single_source` | M21.3 (deploy-time validator) | Engineering — Marketplace | v7.1.1 |
