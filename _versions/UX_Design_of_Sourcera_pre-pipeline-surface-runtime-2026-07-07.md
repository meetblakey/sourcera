...
- Selection Record / Selection Report sources → Master Spec §4.3.21, §10.12.4, §10.13.5; Appendix M rows "Selection Report Draft" and "Selection Report SHA-256 content hash."
...
3. **Mobile.** Layout collapses to single column with the CTA full-width; ≥ 48 px tap target per WCAG 2.1 AA. Sub-headline wraps to two lines on `< 640px`.
...
4. **Mobile.** Card stacks within the requirement's vertical scroll; textarea full-width with ≥ 48 px tap target on the buttons.
...
8. **Mobile.** Tile grid stacks 1-column with ≥ 48px tap targets per §11.6 / WCAG 2.1 AA. The "Other" inline input expands below the tile that was tapped; no other layout changes.
...
| `callout.dismiss.transition` | `motion.duration.medium` (240 ms) ease-out |
...
1. **Render.** The callout renders pinned to the top-right of the §11.3 detail surface's content region with a subtle slide-in motion (240 ms ease-out) on first paint.
...
6. **Dismissal.** Clicking the "Got it" button or the X icon (top-right of the callout) dismisses the callout immediately; reaching the 60-second auto-dismiss timer dismisses it identically. Both paths trigger the slide-out motion (240 ms ease-out) and the callout collapses into a quiet "What we filled in for you" link in the §11.4 persistent UI.
...
8. **Mobile.** The callout renders as a full-width card pinned to the top of the §11.3 mobile layout content region; "Show" / "Change" links and the upgrade affordance retain their behavior; auto-dismiss timer is unchanged. Tap targets ≥ 48px per §11.6 / WCAG 2.1 AA.
...
| State | Trigger | Visual | Notes |
|-------|---------|--------|-------|
| `mounted` | Workspace landed from intake | Callout slides in from right (240 ms) | First paint of the materialized Workspace |
...
| `dismissed-explicit` | Click Got it or X | Slide-out (240 ms); collapses to §11.4 link | |
| `dismissed-auto` | 60s elapsed | Slide-out (240 ms); collapses to §11.4 link | Same animation as explicit |
| `re-opened` | Click §11.4 link | Slide-in (240 ms) without auto-dismiss timer | Stays open until explicit dismiss |
...
| IF-09 | Auto-dismiss fires at 60s elapsed; explicit dismiss fires immediately on Got it / X / Esc. Both paths slide out over 240 ms (instant snap when `prefers-reduced-motion: reduce` is set) and collapse to a §11.4 "What we filled in for you" link. | Visual regression + reduced-motion test |
...
| IF-11 | Mobile (< 640px): tile grid renders 1-column with ≥ 48px tap targets; "Other" inline input expands below the tapped tile; callout renders as full-width card pinned to top of content region; all WCAG 2.1 AA contrast and tap-target rules pass per §11.6. | Mobile responsive test + Axe-core |
...
| `pipeline.step.gap.between` | 24px (desktop), 12px (mobile) | Horizontal gap between step segments. |
| `pipeline.step.transition.duration` | `motion.duration.medium` (240 ms) | Animated slide of the active-step indicator across a step boundary. |
| `pipeline.step.transition.ease` | `motion.ease.standard` | Easing of the indicator slide. |
...
| `pipeline.step.color.connector` | `--color-accent` (active to active+1), `--color-success` (between completed steps), `--color-border` (future segments) | |
...
- `solo` state: four-step bar renders as a single-row segmented control with active step's label in full and other steps as truncated initials (Buyer: S / D / S / D; Seller: R / D / R / S). Tapping a step opens a bottom-sheet detail with the same content as the desktop tooltip plus the step's CTAs. Bottom-sheet token: `bottomSheet.default` per §3.7.
- `team` state: chip ribbon scrolls horizontally; the active chip is auto-scrolled into view on phase change.
- Touch targets meet WCAG 2.1 AA (≥ 48px).
...
| `phase-advance-success` | Advance click with all gates passed (or soft-gate skip) | Active step indicator slides to next step (240 ms); step body re-renders | Both modes |
...
| PS-07 | Step transition motion (active indicator slide across a step boundary) uses `motion.duration.medium` (240 ms) and `motion.ease.standard`. `prefers-reduced-motion: reduce` replaces the slide with an instant snap and disables the intra-step progress fill tween. | Visual regression + reduced-motion test |
...
- Primary metric (display-2 typography per this Spec §2.1): **"$49/mo"** (Annual) or **"$59/mo"** (Monthly), sourced at presentation time from Master Spec §34.2.1 (Buyer) / §34.2.2 (Seller).
...
- Title: **"Eval billed"**.
- Primary metric: **"$199 paid {YYYY-MM-DD}"**.
...
- Primary metric: **"$199 charged {YYYY-MM-DD}"**.
- Secondary line: "7-day refund window · 90-day retention".
- Action chips:
...
- Value-dollar denomination ("$5/mo absorbed", "$0.50 used this week").
...
| DV-03 | Selection Record hash mismatch on read forces regeneration; the "regenerated due to source change" chip renders in the header; PostHog `defense_view.regenerated_due_to_source_change` event fires. | E2E test (mutate Selection Record via Phase 13 → 12 rejection path; verify chip + event) |
