...
### Tablet (640–1024px)
Two-column layout: Sidebar + Main Content. Side Peek opens as a full-width overlay (slides from right, covers main content). Sidebar collapses to icon-only by default; expands on tap. Top navigation bar shows breadcrumb + search icon. Touch targets follow the Master Spec §38.6.2 per-tier minimum.
...
| Side Peek on tablet | Peek width set to 60% viewport width (vs 40% on desktop). Full-width swipe-to-close gesture enabled (right swipe). Header includes back button (chevron-left icon) with label "Back". Backdrop tap also closes. | Master Spec §38.6.2 tablet-tier minimum | Peek never covers entire viewport on tablet. Always shows workspace behind. |
